<?php 
    $rpppk = PerpanjangankontrakModel::whereRaw("status = 1 and sts_kontrak = 2 and MONTH(tr_pppk.tmtawal) = \"".Input::get('bulan')."\" and YEAR(tr_pppk.tmtawal) = \"".Input::get('tahun')."\"")->first();
 ?>
...
<div>
    {!! Form::open(array('url' => url().'/pppk/perpanjangankontrak/simpanskinduk', 'method' => 'POST', 'class'=>'form-horizontal form-'.\Config::get('claravel::ajax'),'id'=>'form-sk-induk')) !!}    
...
<script type="text/javascript">
    ...
    $('#form-sk-induk').on('submit',function(e){
        ...
        bootbox.confirm('Simpan dan Generate Nomor Urut SK Induk?',function(a){
            if(a == true){
                $.ajax({
                    url : '{!! url('') !!}/pppk/perpanjangankontrak/simpanskinduk',
                    type : 'post',
                    data: $this.serialize(),
                    beforeSend: function(){
                        preloader.on();
