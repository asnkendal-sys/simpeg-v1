...
if (session('role_id') <= 3) {
    ...
    $alert = "<strong>Perhatian!</strong><ul><li>Penambahan, Update atau Delete akan diverifikasi oleh Admin BKD terlebih dahulu.</li><li>Riwayat diklat Struktural yang memiliki tmt terbaru akan langsung terupdate ke biodata.</li></ul>";
}
?>
...
<div class="nav-tabs-custom" style="box-shadow:none;">
    ...
    <div class="tab-content">
        <div id="tab_1" class="tab-pane active">
            <div class="row">
                <div class="col-md-12">
                    {!! Form::open(array('url' => url()."/epersonal/biodata/".((session('role_id') <= 3)?'saverdikstru':'saverdikstrutemp'), 'method'=> 'POST', 'files' => true, 'class'=>'form-horizontal form-'.\Config::get('claravel::ajax'),'id'=>'form-rdikstru')) !!}
                        @if(session('role_id') <= 3)
                            {!! Form::hidden('id', null, array('id'=> 'id')) !!}
                            @else
                            {!! Form::hidden('tb', Input::get('tb'), array('id'=> 'tb')) !!}
                            @if((Input::get('tb') == 'r_dikstru') or (Input::get('tb') == ''))
                            {!! Form::hidden('id_rdikstru', null, array('id'=> 'id')) !!}
                            {!! Form::hidden('idjnsaksi', Input::get('flag'), array('id'=> 'idjnsaksi')) !!}
                            @else
                            {!! Form::hidden('id', null, array('id'=> 'id')) !!}
                            {!! Form::hidden('id_rdikstru', null, array('id'=> 'id_rdikstru')) !!}
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#form-rdikstru select').select2();
        $('#form-rdikstru .num').keyup(function() {
            if (this.value != this.value.replace(/[^0-9\.]/g, '')) {
                this.value = this.value.replace(/[^0-9\.]/g, '');
            }
        });
        $("#form-rdikstru .date").mask("99-99-9999");
        $("#form-rdikstru .datepicker").datetimepicker({
            format: 'DD-MM-YYYY'
        });
        $('#form-rdikstru').on('submit', function(e) {
            var $this = $(this);
            e.preventDefault();
        ...
        $('#form-rdikstru #iddikstru').on('change', function(e) {
            e.preventDefault();
            $('#form-rdikstru #dikstru').val($(this).find(":selected").text());
        });
        ...
        <?php if (Input::get("flag") > 1) { ?>
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (ret) {
                        for (attrname in ret) {
                            $('#form-rdikstru #' + attrname).val(ret[attrname]);
                            if ($.inArray(attrname, arrselect2) != -1) {
                                $('#form-rdikstru #' + attrname).val(ret[attrname]).trigger('change.select2');
                            }
                            if ($.inArray(attrname, arrdate) != -1) {
                                var str = ret[attrname];
                                var res = str.split("-");
                                $('#form-rdikstru #' + attrname).val(res[2] + '-' + res[1] + '-' + res[0]);
                            }
                        }
                    }
                }
