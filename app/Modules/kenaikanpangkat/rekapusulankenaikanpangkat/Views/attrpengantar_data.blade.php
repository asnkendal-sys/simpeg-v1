<?php
    ...
    $rs = \DB::table("tr_kenaikan_pangkat")->where('nousul', $nousul)->where('idskpd', 'like', $idskpd. '%');
    // echo $rs->count(); exit();
    if($rs->count() > 0){
        $jml = $rs->count();
        $row = $rs->first();
        // dd($row); exit();
        // echo $row->tgl_sp; exit();
    }else{
...
<div class="row" style="padding-left: 35px;">
    <div class="span12">
        <form id="form2" name="form2" class="form-horizontal" action="<?=url().'/kenaikanpangkat/rekapusulankenaikanpangkat/cetaksimpan/pengantar'?>" target="_blank" class="" method="post" enctype="multipart/form-data">
            {!!csrf_field()!!}
