...
namespace App\Modules\epersonal\biodata\Controllers;
    ...
    public function postDelriwayat()
    {
        ...
        if ($tb == 'r_jab') {
            ...
            if ($idstspeg == '3') {
                ...
                if($rs->idjabbkn == '' || $rs->idjabbkn == null){
                    echo (\DB::table($tb)->where('id', $id)->delete()) ? 9 : 'Data Gagal Dihapus.';
                                    }else{
$send = accessDatadeletesiasn('jabatan/delete', $idRiwayat);
    ...
    public function postSaverpend()
    {
        ...
        if ($validation->passes()) {
            ...
            if ($input['id'] == '') {
                ...
                $id = \DB::table('r_pend')->insertGetId($data);
            } else {
                $data['updated_at'] = gmdate("Y-m-d H:i", time() + 60 * 60 * 7);
            ...
            //candra penamabahan file ijazah

            function cekkode($kode)
            {
                $cek = \DB::connection('efile_2017')->table('files')
                    ->where('kode', '=', $kode)
                    ->count();
                if ($cek > 1) {
                    $pegawai = \DB::table('tb_01')
                        ->select('idskpd')->where('nip', '=', \Input::get('nip'))->first();
                    $kdskpd = str_replace('.', '-', $pegawai->idskpd);
                    $kode = $this->clean($kdskpd) . '-' . strtoupper(str_random(5));
                    // cekkode($kode);
                } else {
                    return $kode;
                }
            ...
            if (Input::hasFile('dokumen_ijazah') != null) {
                $destinationPath = base_path() . '/packages/upload/dokumen/ijazah';
                $mode = 0777;
                $recursive = false;
                $file = Input::file('dokumen_ijazah');
                if ($file != '') {
                    $destinationPath = str_replace("\\", '/', $destinationPath);
                    if (!is_dir($destinationPath)) {
                        mkdir($destinationPath, $mode, $recursive);
                    ...
                    // /*$filename = str_replace(' ', '-', $f->getClientOriginalName());*/
                    // $ctime = mktime(date('H'), date('i'), date('s'), date('m'), date('d'), date('Y'));
                    // $filename = $input['nip'] . "_" . $ctime . "." . $file->getClientOriginalExtension();
                    // @unlink($destinationPath . '/' . $filename);
                    // $file->move($destinationPath, $filename);
                    // $data['files_ijazah'] = '/packages/upload/dokumen/ijazah/' . $filename;

                    // upload ke tabel files dan efile
                    $tgl = date('d-m-Y');
                    $tgl = str_replace('-', '', $tgl);
                    $pegawai = \DB::table('tb_01')
                        ->select('idskpd')->where('nip', '=', \Input::get('nip'))->first();
                    $kdskpd = str_replace('.', '-', $pegawai->idskpd);
                    $kode = cekkode($this->clean($kdskpd) . '-' . strtoupper(str_random(5)));
                    $filename = 'efile-' . \Input::get('nip') . '-' . $tgl . '-' . $kode . '.' . $tipefile;
                    $yearPath = base_path() . '/efile/packages/upload/files/' . substr(\Input::get('nip'), 0, 4);
                    $destinationPath = base_path() . '/efile/packages/upload/files/' . substr(\Input::get('nip'), 0, 4) . '/' . \Input::get('nip');
                    $size = $file->getSize();
                    $type = $file->getMimeType();
                    if (!is_dir($yearPath)) {
                        mkdir($yearPath, $mode, $recursive);
                    }
                    if (!is_dir($destinationPath)) {
                        mkdir($destinationPath, $mode, $recursive);
                    }
                    $uploadSuccess = $file->move($destinationPath, $filename);
                    // $data['files_ijazah'] = $destinationPath . $filename;
                    $data['files_ijazah'] = $filename;
                    $url = url() . '/packages/upload/files/' . substr(\Input::get('nip'), 0, 4) . '/';
                    $thumbnailUrl = url() . '/packages/image.php?width=130&image=/efile/packages/upload/files/' . substr(\Input::get('nip'), 0, 4) . '/' . \Input::get('nip') . '/';
                    if ($uploadSuccess) {
                    ...
                    $id == '' ? $subsubjenis = 0 : $subsubjenis = $id;
                    if (\Input::has('jenis')) {
                        $riwayat_flag = \DB::connection('efile_2017')->table('kategori_jenis')
                            ->where('id', '=', \Input::get('jenis'))
                            ->first()->riwayat_flag;
                    } else {
                        $riwayat_flag = 0;
                    }
                    $datasimpan = array(
                        ...
                        'jenis' => '8', // untuk ijazah selalu 8
                        'subjenis' => $id,
                        'subsubjenis' => $subsubjenis,
                        'riwayat_flag' => $riwayat_flag,
                        'verified' => \Session::get('role_id') == 2 ? 1 : 0,
                    );


                    $simpan = \DB::connection('efile_2017')->table('files')
                        ->insert($datasimpan);
                    // echo ($simpan) ? x : "Data Gagal Disimpan";
                }
            }

            if (Input::hasFile('dokumen_transkrip') != null) {
                $destinationPath = base_path() . '/packages/upload/dokumen/transkrip';
                $mode = 0777;
                $recursive = false;
                $file = Input::file('dokumen_transkrip');
                if ($file != '') {
                    $destinationPath = str_replace("\\", '/', $destinationPath);
                    if (!is_dir($destinationPath)) {
                        mkdir($destinationPath, $mode, $recursive);
                    ...
                    $uploadSuccess = $file->move($destinationPath, $filename);
                    // $data['files_transkrip'] = $destinationPath . $filename;
                    $data['files_transkrip'] = $filename;
                    $url = url() . '/packages/upload/files/' . substr(\Input::get('nip'), 0, 4) . '/';
                    $thumbnailUrl = url() . '/packages/image.php?width=130&image=/efile/packages/upload/files/' . substr(\Input::get('nip'), 0, 4) . '/' . \Input::get('nip') . '/';
                    if ($uploadSuccess) {
                    ...
                    $datasimpan = array(
                        ...
                        'status' => $status,
                        // 'jenis' => \Input::get('jenis'),
                        'jenis' => '77', // untuk transkrip selalu 77
                        'subjenis' => $id,
                        'subsubjenis' => $subsubjenis,
                        'riwayat_flag' => $riwayat_flag,
                        'verified' => \Session::get('role_id') == 2 ? 1 : 0,
            ...
            // end candra penambahan file ijazah

            // echo isset($id) ? '1' : 'Input tidak valid';
        } else {
            echo 'Input tidak valid';
        }
    }
    ...
    public function postSaverpendtemp()
    {
        ...
        //     $mode = 0777;
        //     $recursive = false;
        //     $f = Input::file('dokumen_transkrip');
        //     if ($f != '') {
        //         $destinationPath = str_replace("\\", '/', $destinationPath);
        //         if (!is_dir($destinationPath)) {
        //             mkdir($destinationPath, $mode, $recursive);
        ...
        //         @unlink($destinationPath . '/' . $filename);
        //         $f->move($destinationPath, $filename);
        //         $data['files_transkrip'] = '/packages/upload/dokumen/transkrip/' . $filename;
        //     }
        // } else {
        // }
        // end candra penambahan file ijazah
        ...
        if ($validation->passes()) {
            ...
            if (count($rs) > 0) {
                ...
                $data['created_at'] = gmdate("Y-m-d H:i", time() + 60 * 60 * 7);
                // echo (\DB::table('r_pend')->insert($data)) ? 1 : "Data Gagal Disimpan";
                $id = \DB::table('r_pend_temp')->insertGetId($data);
            }
            ...
            if (Input::hasFile('dokumen_ijazah') != null) {
                ...
                if ($file != '') {
                    ...
                    $simpan = \DB::connection('efile_2017')->table('files_temp')
                        ->insert($datasimpan);
                    // echo ($simpan) ? x : "Data Gagal Disimpan";
                }
            }
    ...
    private function dikstruSerializeData($data)
    {
        ...
        $dataSend  = [
            ...
            'tanggal' => date('d-m-Y', strtotime($data['tgsttpdikstru'])),
            'tanggalSelesai' => date('d-m-Y', strtotime($data['tgsel'])),
    ...
    public function postSaverdikstrutemp()
    {
        ...
        if ($validation->passes()) {
            ...
            if (count($rs) > 0) {
                ...
                $id = \DB::table('r_dikstru_temp')->insertGetId($data);
            }
            //candra penamabahan file dokumen
            ...
            if (Input::hasFile('dokumen_pendukung') != null) {
                // $destinationPath = base_path() . '/packages/upload/dokumen/ijazah';
                $mode = 0777;
                $recursive = false;
                $file = Input::file('dokumen_pendukung');
                if ($file != '') {
                    // $destinationPath = str_replace("\\", '/', $destinationPath);
                    // if (!is_dir($destinationPath)) {
                    //     mkdir($destinationPath, $mode, $recursive);
                    ...
                    $datasimpan = array(
                        ...
                        'status' => $status,
                        // 'jenis' => \Input::get('jenis'),
                        'jenis' => '9', // lihat dari tabel kategori jenis diklat teknis
                        'subjenis' => $id,
                        'subsubjenis' => $subsubjenis,
                        'riwayat_flag' => $riwayat_flag,
                        'verified' => \Session::get('role_id') == 2 ? 1 : 0,
            ...
            // end candra penambahan file dokumen
    ...
    public function postSaverdikfungtemp()
    {
        ...
        if ($validation->passes()) {
            ...
            if (count($rs) > 0) {
                ...
                $id = \DB::table('r_dikfung_temp')->insertGetId($data);
            }
            ...
            if (Input::hasFile('dokumen_pendukung') != null) {
                ...
                if ($file != '') {
                    ...
                    $datasimpan = array(
                        ...
                        'status' => $status,
                        // 'jenis' => \Input::get('jenis'),
                        'jenis' => '10', // lihat dari tabel kategori jenis diklat teknis
                        'subjenis' => $id,
                        'subsubjenis' => $subsubjenis,
                        'riwayat_flag' => $riwayat_flag,
                        'verified' => \Session::get('role_id') == 2 ? 1 : 0,
    ...
    public function postSaverdiktektemp()
    {
        ...
        if ($validation->passes()) {
            ...
            if (count($rs) > 0) {
                ...
                $id = \DB::table('r_diktek_temp')->insertGetId($data);
            }
            //candra penamabahan file dokumen
            ...
            if (Input::hasFile('dokumen_pendukung') != null) {
                ...
                if ($file != '') {
                    ...
                    $datasimpan = array(
                        ...
                        'status' => $status,
                        // 'jenis' => \Input::get('jenis'),
                        'jenis' => '11', // lihat dari tabel kategori jenis diklat teknis
                        'subjenis' => $id,
                        'subsubjenis' => $subsubjenis,
                        'riwayat_flag' => $riwayat_flag,
                        'verified' => \Session::get('role_id') == 2 ? 1 : 0,
    ...
    public function postSaverortu()
    {
        ...
        if ($validation->passes()) {
            ...
            $data['status_jkn']  = Input::get('status_jkn');
            $data['role_id'] = \Session::get('role_id');
            $data['role_id'] = \Session::get('role_id');

            /*update riwayat selain status jkn*/
            if ($data['status_jkn'] == 1) {
                \DB::table('r_ortu')->where('nip', $input['nip'])->where('id', $input['id'])->update(array('status_jkn' => 0));
            }
    ...
    public function postSaveranak()
    {
        ...
        if ($validation->passes()) {
            ...
            /*update riwayat selain status jkn*/
            if ($data['status_jkn'] == 1) {
                \DB::table('r_anak')->where('nip', $input['nip'])->where('id', $input['id'])->update(array('status_jkn' => 0));
            }
            if ($input['id'] == '') {
                $data['created_at'] = gmdate("Y-m-d H:i", time() + 60 * 60 * 7);
    ...
    public function postSaverpppktemp()
    {
        ...
        if ($validation->passes()) {
            ...
            if (count($rs) > 0) {
                ...
                $id = \DB::table('r_pppk_temp')->insertGetId($data);
            }
            ...
            if (Input::hasFile('dokumen_pendukung') != null) {
                ...
                if ($file != '') {
                    ...
                    $datasimpan = array(
                        ...
                        'status' => $status,
                        // 'jenis' => \Input::get('jenis'),
                        'jenis' => '6', // lihat dari tabel kategori jenis diklat teknis
                        'subjenis' => $id,
                        'subsubjenis' => $subsubjenis,
                        'riwayat_flag' => $riwayat_flag,
                        'verified' => \Session::get('role_id') == 2 ? 1 : 0,
    ...
    private function jabatanSerializeDatasiasn($data)
    {
        ...
        } elseif (!empty($data['idtugasdoktergigi'])) {
            $subJabatanId = \DB::table('a_tugasdokter')->where('idtugasdokter', '=', $data['idtugasdoktergigi'])->first()->idsiasn;
