<script type="text/javascript">
    $(document).ready(function(){
        ...
        <?php if(session('role_id') <= 3){?>
            $('#kolektif #idskpd').select2();
            $('#kolektif #idskpd').on('change', function(e){
                e.preventDefault();
                var idskpd = $('#kolektif #idskpd').val();
                var jnssurat = $('#kolektif #jnssurat').val();
                ...
                $.ajax({
                    ...
                    success:function(response){
                        ...
                        $('#kolektif #name').val(ret.name);
                        $('#kolektif #template').val(ret.template);
                    }
                });
            });
        <?php } ?>

        $('#kolektif .ckeditor').ckeditor(config_pengantar);

        $('#kolektif #form1').on('submit',function(e){
            var $this = $(this);
            e.preventDefault();
        ...
        $('#kolektif #defaultthem').on('click', function(e){
            e.preventDefault();
            $.ajax({
                type : 'post',
                url : '{!!url()!!}/emutasi/templateskpengangkatan/surat',
                ...
                success:function(response){
                    preloader.off();
                    var ret = $.parseJSON(response);
                    $('#kolektif #template3').val(ret.template);
                }
            });
        });
    });
        ...
        <div class="span12">
            <div class="accordion-heading">
                <div class="control-group">
                    ...
                    @endif
                    <input type="hidden" name="jnssurat" class="jnssurat" id="jnssurat" value="5.2">
                </div>

                <div class="control-group">
                    <label class="control-label" for="jnssurat">Nama Surat</label>
                    <div class="controls">
                        <input type="text" name="nama" class="nama input-xxlarge form-control" id="nama" value="<?php echo TemplateskpengangkatanModel::getTemplate('all', '5.2','nama')?>">
                    </div>
                </div>
                ...
                <div class="control-group">
                    ...
                    <div class="controls">
                        <textarea rows="10" cols="600" name="template3" class="ckeditor" id="template3" placeholder="Template Surat Kolektif">
                          <?php
                          if(TemplateskpengangkatanModel::getTemplate(session('idskpd'), '5.2','template')=='0'){
                            echo TemplateskpengangkatanModel::getTemplate('all', '5.2','template');
                        }else{
                            echo TemplateskpengangkatanModel::getTemplate(session('idskpd'), '5.2','template');
                        }
                        ?>
                    </textarea>
                </div>
