<style type='text/css'>
    ...
    table tbody td div.r,
    table tfoot td div.r,
    table tfoot th div.r {
        text-align: right;
    }
</style>
...
<br>
<div align="center">
    <h4>NOMINATIF PERPANJANGAN KONTRAK PEGAWAI PPPK PW</h4>
    <h4>
                ...
                <td colspan="15">
                    <table border="1" rules="rows" style="background: {!!(($tahun_penjagaan>0)?'#ececec':(($item->selisih_bulankerja>0)?'#90ee90':'#FAFAD2'))!!}">
                        ...
                        <tr>
                            ...
                            <td class="text-center">{!!$item->mkthnakhir_pppk + $item->selisih_tahunkerja !!}</td>
    ...
    $(document).ready(function() {
        ...
        for (let i = 1; i <= n; i++) {
            ...
            $('.preview' + i).on('click', function() {
                var nip = $(this).attr('recnip');
                claravel_modal('Preview Berkas Pepanjangan PPPK PW', 'Loading...', 'main_modal2');
                $.ajax({
                    url: "{{ url('') }}" + '/pppkpw/perpanjangankontrakpw/data/berkas',
                    type: 'post',
                    data: {
                        'nip': nip,
                        'sts_kontrak': 2,
