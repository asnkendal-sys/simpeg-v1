<?php namespace App\Modules\webservices\datapensiun\Controllers;

use App\Http\Controllers\Controller;
use App\Modules\webservices\datapensiun\Models\DatapensiunModel;
use Input,View, Request, Form, File;

/**
 * Datapensiun Controller
 * @var Datapensiun
...
class DatapensiunController extends Controller {
    protected $datapensiun;

    public function __construct(DatapensiunModel $datapensiun){
        $this->datapensiun = $datapensiun;
    }
    ...
    public function getIndex(){
        ...
        if ((strlen(Input::has('search')) > 0) or (Input::get('idskpd') != '') or (Input::get('idstspeg') != '')) {
            ...
            $datapensiuns = $this->datapensiun->all();
        }
        return View::make('datapensiun::index', compact('datapensiuns'));
    }


    public function getCreate(){
        cekAjax();
        return View::make('datapensiun::create');
    }

    public function postCreate(){
        cekAjax();
        $input = Input::all();
        $validation = \Validator::make($input, DatapensiunModel::$rules);
        if ($validation->passes()){
            $input['user_id'] = \Session::get('user_id');
            $input['role_id'] = \Session::get('role_id');
            echo ($this->datapensiun->create($input))?1:"Gagal Disimpan";
        }
        else{
    ...
    public function getEdit($id = false){
        ...
        $datapensiun = $this->datapensiun->find($id);
        //if (is_null($datapensiun)){return \Redirect::to('webservices/datapensiun/index');}
        return View::make('datapensiun::edit', compact('datapensiun'));
    }
    ...
    public function postEdit(){
        ...
        if ($validation->passes()){
            $datapensiun = $this->datapensiun->find($id);
            echo ($datapensiun->update($input))?4:"Gagal Disimpan";
        }
        else{
            echo 'Input tidak valid';
        }
    ...
    public function postDelete(){
        ...
        if (is_array($ids)){
            foreach($ids as $id){
                $this->datapensiun->find($id)->delete();
            }
            echo 'Data berhasil dihapus';
        }
        else{
            echo ($this->datapensiun->find($ids)->delete())?9:'Gagal Dihapus';
        }
    }
    ...
    function postData(){
        ...
        return View::make('datapensiun::'.$view.'_data');
    }

    function postExcel(){
        $view = Request::segment(4);
        return View::make('datapensiun::'.$view.'_excel');
    }
