...
<script>
	$(document).ready(function(){
		$('#tb-statistik .link').click(function(){
			...
			var vidtingkat = $(par).find('.ed1').html();
			var vidskpd = $(par).find('.ed2').html();
            //alert("Index : "+index+" - Par : "+par+" - idgolru : "+vidgolru+" - idtingkat : "+vidtingkat+" - idskpd : "+vidskpd);
...
?>
<form id="form-print" name="form-print" action="<?=base_url()?>epersonal/page/view/statistik_jabfung_print" class="form-horizontal" method="post" enctype="multipart/form-data" target="_blank">
...
<h4 align="center">STATISTIK PEGAWAI BERDASARKAN JABATAN FUNGSIONAL <?=strtoupper(($item->skpd=='')?'':' PADA '.$item->skpd)?></h4><br>
<table class="table table-hovered table-bordered" id="tb-statistik" border="<?php echo ($this->uri->segment(3)=='view')?'1':'0';?>">
	<thead class="breadcrumb">
		<tr>
			<th rowspan="3"><div class="text-center">JABATAN FUNGSIONAL</div></th>
	...
	<tbody>
		...
		$rs = $this->db->query("
            ...
            FROM a_jabfung a
            LEFT JOIN tb_01 c ON a.idjabfung=c.idjabfung AND idjenjab = 2
            WHERE c.idjenkedudupeg NOT IN('21','99') $whereskpd
            GROUP BY LEFT(c.idjabfung,3)
            ORDER BY a.jabfung2
		");
		$n = 0;
		...
		<tr>
			<td>
			<span class="ed1" style="display:none"><?=$item->tingkat?></span>
			<span class="ed2" style="display:none"><?=$this->input->post('idskpd')?></span>
			<div class="text-left"><?=$item->jabfung2?></div></td>
