...
<?php
    ...
    $rs = \DB::table('tb_01')
        ->select('tb_01.*','a_esl.idgolrumax','a_tkpendid.maxgol','a_golruang.golru','a_golruang.golru_p3k','a_skpd.path_short','a_esl.esl','a_tkpendid.tkpendid','a_jenjurusan.jenjurusan','a_jenjab.jenjab','a_dikstru.dikstru',
...
<br><div align="center">
<h4>DAFTAR NOMINATIF KENAIKAN PANGKAT REGULER</h4>
<h4>
    <?php
        echo ((Input::get('idskpd') != '')?'PADA '.strtoupper(getSkpd(Input::get('idskpd'))):'');
        echo " ".strtoupper(formatBulan(Input::get('blnkpr')));
...
<table border="1" width="100%" id="table table-striped table-hover table-condensed table-bordered">
  ...
  <tbody>
	...
	foreach($rs as $item){
        ...
        if($item->idjenjab>4){
            $color = (($golru_next>$item->idgolrumax)?'yellow':''); //sruktural
        }else{
            $color = (($golru_next>$item->maxgol)?'yellow':''); //fungsional, pelaksana
        }
	?>
    <tr style="background-color: {!!$color!!};">
        ...
        <td align="center">
            ...
            @if($item->idjenjab == 2)
                <div class="text-left">PAK : {!!$item->nopak!!}</div>
            @endif
        </td>
        <td align="center">
