...
<body>
    ...
    $template = TemplateskpengangkatanModel::getTemplate(session('idskpd')!=''?session('idskpd'):'all', '5.1', 'template');
    $where = "(tr_mutasi_pengangkatan.statususul = 1 or tr_mutasi_pengangkatan.statussk = 1)";
    $where .= "and tr_mutasi_pengangkatan.nip =\"".\Request::segment(6)."\" and tr_mutasi_pengangkatan.nousul = \"".\Request::segment(5)."\"";
    $count = \DB::table("tr_mutasi_pengangkatan")->whereRaw($where)->count();


    $item = \DB::table('tr_mutasi_pengangkatan')
    ->select('tr_mutasi_pengangkatan.*','a_golruang.golru','a_golruang.pangkat','a_tkpendid.tkpendid','a_jenjurusan.jenjurusan','skpdlama.path_short as skpdlama','skpdbaru.path_short as skpdbaru'
        ,'tb_01.tmlhr','tb_01.tglhr',
        \DB::raw('IF(LEFT(skpdlama.idskpd,2)=05 OR LEFT(skpdlama.idskpd,2)=04 OR LEFT(skpdlama.idskpd,2)=26,skpdlama.path,CONCAT(skpdlama.skpd," ",skpdlama.path_short)) as skpdlama'),
        \DB::raw('IF(LEFT(skpdbaru.idskpd,2)=05 OR LEFT(skpdlama.idskpd,2)=04 OR LEFT(skpdlama.idskpd,2)=26,skpdbaru.path,CONCAT(skpdbaru.skpd," ",skpdbaru.path_short)) as skpdbaru'),
    ...
    $arrsearch = array("search","[no_suratrek]","[surat_dari]","[tgl_suratrek]","[perihalrek]","[nosk]","[urutan]","[namalengkap]","[tkpendid]","[nip]","[tmlhr]","[tglhr]","[pangkat]","[gol]","[jablama]","[skpdlama]","[jabbaru]",
        "[skpdbaru]","[tgl_sp]","[jurusan]","[tmt]","[tglsurat]","[jabatansekda]","[namasekda]","[pangkatsekda]","[nipsekda]","[kepalabkd]","[pangkatbkd]","[nipkepalabkd]","[jabkepalabkd]","[bupati]","[tembusan_skpd_baru]","[tembusan]","[copyright]","[qrcode]","[tgl_berlaku]",
    ...
    $arrreplace = array("replace",
        ...
        ucword($item->skpdlama),

        $item->jabatanbaru." ".NominatifpengangkatanModel::isSekdes($item->nip,$item->idjabfungumbaru),
        ucword($item->skpdbaru),
        ...
        $item->pangkatsekda,
        fnip($item->nipsekda),
        $item->kepalabkd,ucword($item->pangkatbkd),fnip($item->nipkepalabkd),
        $item->jabkepalabkd,$item->bupati,
        '',
        getTembusanDalamOPD($item->idskpd,$item->idskpdbaru),
        $key_rand,'<div id="qrcode"></div>',formatTanggalPanjang($item->tmt),
        $item->kepalabkd,
