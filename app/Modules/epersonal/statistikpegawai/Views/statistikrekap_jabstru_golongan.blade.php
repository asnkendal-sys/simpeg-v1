...
<form id="form-print" name="form-print" action="{!!url()!!}/epersonal/statistikpegawai/print/statistikrekap_jabstru_golongan" class="form-horizontal" method="post" enctype="multipart/form-data" target="_blank">
    {!!csrf_field()!!}
	<input type="hidden" id="idgolru" name="idgolru" value="" />
  <input type="hidden" id="idjenjab" name="idjenjab" value="" />
</form>
<h4 align="center">REKAPITULASI JABATAN STRUKTURAL BERDASARKAN GOLONGAN RUANG PER - {{strtoupper(formatBulan(date('m')))}} {{ date('Y')}}</h4><br>
	...
	<tbody>
        ...
        $rs = \DB::table('a_golruang as a')
            ->select(
                ...
                \DB::raw("SUM(IF(a.idgolru=b.idgolrupkt AND b.idjenjab>='20',1,0)) AS 'jumlah'"),
                \DB::raw("SUM(if(left(b.idgolrupkt,1)='1' AND b.idjenjab>='20',1,0)) as 'satu'"),
                \DB::raw("SUM(if(left(b.idgolrupkt,1)='2' AND b.idjenjab>='20',1,0)) as 'dua'"),
                \DB::raw("SUM(if(left(b.idgolrupkt,1)='3' AND b.idjenjab>='20',1,0)) as 'tiga'"),
                \DB::raw("SUM(if(left(b.idgolrupkt,1)='4' AND b.idjenjab>='20',1,0)) as 'empat'")
            )
