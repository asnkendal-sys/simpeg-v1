<?php
    ...
    if ((strlen(Input::has('search')) > 0) or (Input::get('idskpd') != '') or (Input::has('tahun') != '') or (Input::has('bulan') != '') or (Input::has('status') != '') or (Input::has('statussk') != '') or (Input::get('status_tte') != '')) {                    
        ...
        if(Input::get('status') != ''){
            $where .= " and tr_pppk.status = \"".Input::get('status')."\"";
        ...
        $rs = PerpanjangankontrakModel::select(
                    'tr_pppk.*', 'tb_01.alm', 'tb_01.almrt', 'tb_01.almrw', 'tb_01.almdesa', 'tb_01.almkec', 'tb_01.almkab', 'tb_01.almprov', 'tb_01.almkdpos',
                ...
                ->whereRaw($where)->orderBy('status')->orderBy('no_urut')->orderBy('tglhr')->orderBy('idgolru', 'desc')->orderBy('nama')
                ->get();
    }else{
        $rs = PerpanjangankontrakModel::select('tr_pppk.*', 'tb_01.alm', 'tb_01.almrt', 'tb_01.almrw', 'tb_01.almdesa', 'tb_01.almkec', 'tb_01.almkab', 'tb_01.almprov', 'tb_01.almkdpos',
                \DB::raw("DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(tr_pppk.tglhr)), '%Y%m')+0 AS usia"),
                \DB::raw("TIMESTAMPDIFF(YEAR, tmtawall, tmtakhirl) AS selisih_tahunkerja"),
                \DB::raw("TIMESTAMPDIFF(MONTH, tmtakhirl, bup) AS selisih_bulankerja")
            )
 ...
 <Worksheet ss:Name=\"Sheet1\">
  <Table ss:ExpandedColumnCount=\"25000000\" ss:ExpandedRowCount=\"7000000\" x:FullColumns=\"1\"
   ...
   <Row ss:AutoFitHeight=\"0\" ss:Height=\"29.25\">
    <Cell ss:Index=\"16\" ss:StyleID=\"s67\"><Data ss:Type=\"String\">AWAL</Data></Cell>
    <Cell ss:StyleID=\"s67\"><Data ss:Type=\"String\">AKHIR</Data></Cell>
    <Cell ss:Index=\"21\" ss:StyleID=\"s67\"><Data ss:Type=\"String\">KONTRAK</Data></Cell>
