...
<body>
    ...
    if($periode_tgl1!='' && $periode_tgl2!=''){
        ...
        $pengangkatans = \DB::table('tr_mutasi_pengangkatan')
                ->select('idusul')
                ->whereRaw($where)
        ...
        foreach ($pengangkatans as $pengangkatan) {
            $x++;
            \DB::table('tr_mutasi_pengangkatan')->where('idusul', $pengangkatan->idusul)->update(array('urutan' => $x ));
        }*/
    } else{
    ...
    ->select('tr_mutasi_pengangkatan.*','a_golruang.golru','a_tkpendid.tkpendid','a_jenjurusan.jenjurusan'
        ,'skpdlama.skpd as skpdlama','skpdbaru.path as skpdbaru','tb_01.tmlhr','tb_01.tglhr','a_golruang.pangkat',
    ...
    ->leftJoin('a_skpd as skpdbaru', \DB::raw('left(tr_mutasi_pengangkatan.idskpdbaru,2)'), '=', 'skpdbaru.idskpd')
    ...
    <div id="F4-landscape">
        <table border="0" cellpadding="0" width="100%">
            <tr>
                ...
                <td colspan="3">KEPUTUSAN BUPATI KENDAL <?php /*echo getSkpdgroupMutdalam(substr($row->idskpd,0,2))*/?></td>
            </tr>
            <tr>
                <td width="70%" colspan="3">&nbsp;</td>
                <td width="6%">NOMOR</td>
                <td width="2%" align="center">:</td>
                <!-- INI --><td><?php echo ($row->nosk!='')?$row->nosk:'820/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/31'?></td>
            </tr>
            <tr>
                <td width="70%" colspan="3">&nbsp;</td>
                <td width="6%">TANGGAL</td>
                <td width="2%" align="center">:</td>
                <td><?php echo strtoupper(formatTanggalPanjang($tgl_sp))?></td>
            </tr>
            <tr>
                <td colspan="6" align="center"><br><p>PENYESUAIAN DALAM NOMENKLATUR JABATAN PELAKSANA BAGI PEGAWAI NEGERI SIPIL DI LINGKUNGAN PEMERINTAH KABUPATEN KENDAL</p></td>
            </tr>
        </table>
        <br>
            ...
            <tbody>
                ...
                <?php
                    ...
                    foreach($item as $item){
                        $i++;
                        \DB::table('tr_mutasi_pengangkatan')->where('idusul', $item->idusul)->update(array('urutan' => $i ));
                ?>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td align="center" width="50%">
                            <div style="float: right; width: 50%; margin-right: 1em;">
                                ...
                                <br />
                                <b>
                                    <!--<u><?php /*echo @$attr->nama*/?></u><br />
                                    <?php /*echo ucword(@$attr->pangkat)*/?><br />
                                    --><?php /*echo "NIP. ".@$attr->nip*/?>
                                    <!--<u>{!!@$row->bupati!!}</u>-->
                                </b>
                                MIRNA ANNISA
                                </div>
