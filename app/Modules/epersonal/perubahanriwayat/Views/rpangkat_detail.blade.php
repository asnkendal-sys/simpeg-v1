      ...
      {!! Form::open(array('url' => \Request::path().'/delete', 'method' => 'POST', 'class' => 'form-'.\Config::get('claravel::ajax'),'id'=>'data' )) !!}
                  ...
                  <p>
                     <div class="nav-tabs-custom" style="box-shadow:none;">
                        <ul class="nav nav-tabs tab2" id="myTabs">
                           ...
                           <li class="active"><a data-toggle="tab" href="#" id="rpangkat" recnip="{!! Input::get('nip') !!}"><i class="fa fa-fw fa-dot-circle-o"></i> PANGKAT</a></li>
                           ...
                           <div id="pangkat" class="tab-pane active">
                              <p>
                                 <table class="table table-striped table-hover table-condensed table-bordered" role='grid' id="tb-rpangkat">
                                    ...
                                    @foreach ($rpangkats as $rpangkat)
                                    <?php $x++; ?>
                                    <tr>
                                        <td rowspan="2"align="center">{!!$x!!}</td>
                                        <td align="left">{!!$rpangkat->golru." - ".$rpangkat->pangkat!!}</td>
                                        <td>{!!($rpangkat->jabatan!='')?$rpangkat->jabatan:$rpangkat->pejmenpkttext!!}</td>
                                        <td align="left">{!!$rpangkat->nosk!!}</td>
                                        <td align="center">{!!date('d-m-Y', strtotime($rpangkat->tgsk))!!}</td>
                                        <td align="center">{!!date('d-m-Y', strtotime($rpangkat->tmtpkt))!!}</td>
                                        <td rowspan="2" align="right">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu pull-right">
                                                    <li><a class="text-info prevperubahan2" recid="{!!$rpangkat->id!!}" recnip="{!!$rpangkat->nip!!}" recflag="{!!$rpangkat->idjnsaksi!!}" href="javascript:void(0)" ><i class="{!!(session('role_id') <= 3)?'fa fa-check':'fa fa-search'!!}"></i> {!!(session('role_id') <= 3)?'Verifikasi':'Preview'!!}</a></li>
                                                    <li><a class="text-danger btlperubahan" recid="{!!$rpangkat->id!!}" recnip="{!!$rpangkat->nip!!}" recflag="{!!$rpangkat->idjnsaksi!!}" href="javascript:void(0)" ><i class="fa fa-times-circle"></i> Batalkan</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td><em><small>{!!$rpangkat->nip."<br>".$rpangkat->namalengkap!!}</small></em></td>
                                        <td><em><small>{!!$rpangkat->path!!}</small></em></td>
                                        <td colspan="3" class="{!!($rpangkat->status == 2)?'alert-danger':''!!}">
                                            <em><small>{!!getKetaksi($rpangkat->idjnsaksi)!!}</small></em><br>
                                            <em><small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Keterangan : {!!($rpangkat->ketditolak!='')?$rpangkat->ketditolak:'Belum ada tanggapan.'!!}</small></em>
                                        </td>
                                    </tr>
                                    @endforeach
                                    </tbody>
         ...
         <div class="box-footer clearfix">
            <div class="row">
               ...
               <div class="col-sm-6">
                   <?php echo $rpangkats->appends(array('idskpd'=>Input::get('idskpd'), 'search' => Input::get('search')))->render(); ?>
               </div>
            </div>
         </div>
      {!! Form::close() !!}
               ...
               <div class="box-body">
                  ...
                  <div class="nav-tabs-custom" style="box-shadow:none;">
                     <ul class="nav nav-tabs tab1" id="myTab2">
                        <li class="active"><a data-toggle="tab" href="#pangkatakhir"> <i class="fa fa-fw fa-anchor"></i> DAFTAR PANGKAT</a></li>
                     </ul>
...
<script>
   $(document).ready(function(){
      ...
      $('.prevperubahan2').on('click', function(e){
          e.preventDefault(e);
          claravel_modal('Perubahan Riwayat Pangkat','Loading...','main_modal2');
          $.ajax({
             type:'post',
             url : '{!!url()!!}/epersonal/perubahanriwayat/data/rpangkat_perubahan',
          ...
          bootbox.confirm('Batalkan perubahan data ?',function(a){
             if(a == true){
                 $.ajax({
                      ...
                      type : 'post',
                      data: {'id' : $this.attr('recid'), 'tb':'r_gol_temp', '_token' : '{!!csrf_token()!!}'},
                      beforeSend: function(){
                          preloader.on();
                      },
                      success:function(html){
