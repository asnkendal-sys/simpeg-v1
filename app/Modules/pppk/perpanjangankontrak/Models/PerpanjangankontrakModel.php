<?php namespace App\Modules\pppk\perpanjangankontrak\Models;
use Illuminate\Database\Eloquent\Model;
use App\Models\PPPK\UsulanPPPK;

class PerpanjangankontrakModel extends UsulanPPPK {
    ...
    public static function cekDokumen($nip="", $sts_kontrak=""){
        ...
        if($sts_kontrak == 2){
            $cek2 = \DB::connection('efile_2017')->table('files')
                ...
                ->get();
            $message .= (count($cek2) > 1 and $cek2[0]->tahun >= (date('Y') - 1))?'':' Berkas Kinerja 2 tahun terakhir belum tersedia.';
...
//            $message .= ($cek3 > 0)?'':' Perjanjian Kerja belum tersedia.';
            ...
            $cek5 = \DB::connection('efile_2017')->table('files')
                ...
                ->join('kategori_jenis AS kat', 'kat.id', '=', 'files.jenis')
                ->where('files.nip', $nip)
                ->where('files.subjenis', '123')
                ->groupBy('kat.id', 'files.subjenis')
                ->count();

            $message .= ($cek5 > 0)?'':' Surat Rekomendasi OPD belum tersedia.';
        }
    ...
    public static function cekSkinduk($bulan="", $tahun=""){        
        $rs = \DB::table('tr_pppk')
        ->whereRaw("status = 1 and sts_kontrak = 2 and statussk != 1 and MONTH(tr_pppk.tmtawal) = \"".$bulan."\" and YEAR(tr_pppk.tmtawal) = \"".$tahun."\"")        
        ->count();
