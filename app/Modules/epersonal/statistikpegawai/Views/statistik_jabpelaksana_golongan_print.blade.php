...
<?php
    $where = " tb_01.idjenkedudupeg not in (99,21) and tb_01.idjenjab = 3 and tb_01.idstspeg in (".Input::get('idstspeg').")";
    $skpd = getSkpd(Input::get('idskpd'));
    $golru = getAttr('a_golruang', 'idgolru', Input::get('idgolru'), 'golru');
    $jabfungum = getAttr('a_jabfungum', 'idjabfungum', Input::get('idjabfungum'), 'jabfungum');

    switch(Input::get('idjabfungum')){
        case "":
            $where .= "";
            break;
        default:
...
<h3>BERDASARKAN JABATAN PELAKSANA <!-- {!!$jabfungum!!} --></h3>
</div><br>
    <table border="1">
