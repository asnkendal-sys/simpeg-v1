...
<body>
    ...
    // $cekTemplate =  \DB::table('tr_ijin_cuti_template_sk')->whereRaw("jnssurat = '1.1' AND idskpd = '".session('idskpd')."' ")->first();
    // if(count($cekTemplate) > 0){
    //     $template = TemplateskcutiModel::getTemplateCuti(session('idskpd'), '1.1', 'template');
    // }else{
    ...
    $where = "tr_ijin_cuti.nousul = \"".$nousul."\" AND view_kuota_cuti.nousul = \"".$nousul."\"";
    $where .= " AND tr_ijin_cuti.wewenang_status = 1 ";


    $rs = \DB::table('tr_ijin_cuti')
    ->leftJoin('view_kuota_cuti', function($join)
    {
        $join->on('view_kuota_cuti.nousul', '=', 'tr_ijin_cuti.nousul');
    })
    ->whereRaw($where)->get();
    // dd($item);// exit();
    $i = 0;    
...
$(document).ready(function() {
    $("#qrcode{!!$i!!} ").qrcode({
        ...
        height: 105.82677165,
        render: "image",
        text: "{!!url().'/digitalsigns/ecuti/'.$item->nip.'/'.$item->nousul!!}"
    });
