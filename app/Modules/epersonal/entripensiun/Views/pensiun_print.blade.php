...
<head>
    <title>{!!ucfirsts(getUtility('alias_aplikasi'))!!} Nominatif Pensiun</title>
    ...
    $where = " tb_01.idjenkedudupeg in('99','21') and tb_01.nip != '' ";
        // if (Input::has('search') or Input::has('idskpd') or Input::has('tahun') or Input::has('bulan1') or Input::has('bulan2')) {
            ...
            if(Input::get('idjenpens') != ''){
                $where.= "and tb_01.idjenpens like '".Input::get('idjenpens')."%'";
            }
...
?>
            <div align="center">
                ...
                <h3>
                    {!!((Input::get('idskpd') != '')?'PADA '.strtoupper(getSkpd(Input::get('idskpd'))):'')!!} @if((Input::get('bulan1') != '')
                    and (Input::get('bulan2') != '')) BULAN {!!strtoupper(formatBulan(Input::get('bulan1')))!!} - BULAN {!!strtoupper(formatBulan(Input::get('bulan2')))!!}
                    @elseif ((Input::get('bulan1') == '') and (Input::get('bulan2') != '')) BULAN {!!strtoupper(formatBulan(Input::get('bulan2')))!!}
                    @elseif ((Input::get('bulan1') != '') and (Input::get('bulan2') == '')) BULAN {!!strtoupper(formatBulan(Input::get('bulan1')))!!}
            ...
            <table border="1" width="100%" id="table table-striped table-hover table-condensed table-bordered">
                <thead class="bg-primary">
                    <tr>
                        ...
                        <th rowspan="2">
                            <div class="text-left">TMT DAN</div>
                            <div class="text-left">USIA PENSIUN</div>
                        </th>
                    </tr>
                    <tr>
                        <th>
...
?>
                        <tr>
                            ...
                            <td>
                                <div class="text-left">{!!$entripensiun->namalengkap!!} </div>
                                <small><div class="text-left">{!!$entripensiun->tmlhr.", ".(($entripensiun->tglhr != '0000-00-00')?date('d-m-Y', strtotime($entripensiun->tglhr)):'-')!!}</div></small>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!$entripensiun->nip!!}</div>
                                <div class="text-center">{!!$entripensiun->niplama!!}</div>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!$entripensiun->golru!!}</div>
                                <div class="text-center">{!!(($entripensiun->tmtpkt!='0000-00-00')?date('d-m-Y', strtotime($entripensiun->tmtpkt)):'')!!}</div>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!($entripensiun->esl!='')?$entripensiun->esl:'-'!!}</div>
                            </td>
                            <td>
                                <small>
                <div class="text-left">{!!strtoupper(($entripensiun->jabatan!='')?$entripensiun->jabatan:'-')!!}</div>
                <div class="text-left"><i>Pada</i></div>
                <div class="text-left">{!! (($entripensiun->path_short !='-')?$entripensiun->path_short:'') !!}</div>
                <div class="text-left">{!! (($entripensiun->tmtjbt != '0000-00-00')?date('d-m-Y', strtotime($entripensiun->tmtjbt)):'')!!}</div>
            </small>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!$entripensiun->mkthnpkt!!}</div>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!$entripensiun->mkblnpkt!!}</div>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!substr($entripensiun->mkskr,0,-2)!!}</div>
                            </td>
                            <td align="center">
                                <div class="text-center">{!!substr($entripensiun->mkskr,-2)!!}</div>
                            </td>
                            <td>
                                <div class="text-left">{!!$entripensiun->tkpendid !!}</div>
                            </td>
                            <td>
                                <div class="text-left">{!!$entripensiun->jenjurusan!!}</div>
                            </td>
                            <td align="center">
                                @if($entripensiun->usia!='')
                                <div class="text-center">{!!$entripensiun->agama !!} </div>
                                <div> {!! substr($entripensiun->usia,0,2)." thn ".substr($entripensiun->usia,2,2)." bln"!!}</div>
                                @else
                                <div class="text-center">{!!$entripensiun->agama !!} </div>
                                <div> 0 thn 0 bln</div>
                                @endif
                            ...
                            <td align="center">
                                <div class="text-center">{!!($entripensiun->tmtpens!='0000-00-00')?"TMT : ".date('d-m-Y', strtotime($entripensiun->tmtpens))."<br/>":'-'!!}</div>
                                <div class="text-center">{!!$entripensiun->usiapens!!} thn</div>
                            </td>
                        </tr>
                        <?php }
    } else { ?>
