...
->select(
	...
	'a.idgolrupkt',
	'b.skpd','a_golruang.golru','a_golruang.pangkat','a.idstspeg',
	...
	<div class="box box-primary">
		<div class="row">
			<div class="col-md-12">
                ...
                @if($item->idstspeg == 3)
                <input type="hidden" name="mscpn_thn" value="{!!substr($item->mkskr,0,-2)!!}">
                <input type="hidden" name="mscpn_bln" value="{!!ltrim(substr($item->mkskr,-2),'0')!!}">
								...
								<div class="col-md-6" style="margin-left: -10px;">
									<table class="table table-hovered table-stripped" width="100%">
										...
										<tr class="adariwayat">
											...
											<td class="text-center" width="2%"> : </td>
											<td>
												<!-- {!!comboJenisCuti('id_jenis_cuti')!!} by ramdani 270224-->
												@if(session('role_id')==5)
													{!!NominatifcutiModel::comboJenisCutiPgw('id_jenis_cuti','','','id_jenis_cuti',$item->idstspeg,$item->nip)!!}
												@else
			...
			$(".datepicker1, .datepicker2").on('dp.change', function (e) {
				...
				if(tgl_mulai != "" && tgl_selesai != ""){
					$.ajax({
						...
						type : 'POST',
						data: {'nip':"{!! $nip !!}", 'tgl_mulai': tgl_mulai,'tgl_selesai': tgl_selesai, 'jen_cuti': $('#id_jenis_cuti').val(), '_token' : '{!!csrf_token()!!}'},
						success:function(response){
							var ret = $.parseJSON(response);
