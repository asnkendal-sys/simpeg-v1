...
function combo_ratingHasil($id = '', $selected = "", $required = "")
{
    ...
    $h .= '<option ' . (($selected == 'Dibawah Ekspektasi') ? 'selected' : '') . ' value="Dibawah Ekspektasi">Dibawah Ekspektasi</option>';
    $h .= '</select>';
    return $h;
}
...
function getGaji($golongan, $thmasker, $stspeg = '')
{
    ...
    $cek = \DB::table($table)->select(\DB::raw('max(MSK) as mskmax'))->where('pkt', $golongan)->where('status', 1)->first();
    if (count($cek) > 0) {
        ...
        $rsgaji = \DB::table($table)->select('gaji')->where('pkt', $golongan)->where('status', 1)->where('msk', $masaker)->first();
...
function ucword($string)
{
    ...
    $excluded = ['dan', 'dengan', 'di', 'yang', 'dari', 'ke', 'untuk', 'pada', 'atau'];

    // Ambil list dari database untuk kata yang HARUS uppercase
    $rs = \DB::table('a_ucword')->get();
    foreach ($rs as $item) {
        $x++;
        $rsword .= $item->ucword . (($x != count($rs)) ? "|" : "");
    ...
    // uppercase-kan kata yang ada di database
    if (!empty($rsword)) {
    ...
    // revert kembali kata yang tidak boleh capital
    $result = preg_replace_callback(
        "/\b(" . implode('|', $excluded) . ")\b/i",
        function ($m) {
...
function comboStspns($id = "idstspeg", $sel = "", $required = "")
{
    ...
    $ret .= "<option value=\"4\" " . (($sel == '4') ? 'selected' : '') . ">PPPK PARUH WAKTU</option>";
    $ret .= "</select>";
    return $ret;
}
...
function btnCreatekgb($caption = '')
{
    if ($caption == 'PNS') {
        return "<a id='buatkgbpns' href='" . url() . '/' . \Request::path() . '/create' . "' class='btn" . \Config::get('claravel::ajax') . "'><i class='fa fa-bookmark'></i> $caption</a><br>";
    } else {
        return "<a id='buatkgbp3k' href='" . url() . '/' . \Request::path() . '/createp3k' . "' class='btn" . \Config::get('claravel::ajax') . "'><i class='fa fa-bookmark-o'></i> $caption</a>";
    }
}
...
function getTokenSSO($client_id, $username, $password, $mode = 'train')
{
    ...
    if (($jsondata = curl_exec($curl)) === false) {
        ...
        if (isset($obj['access_token'])) {
            ...
            fwrite($token_file, $expired_time);
            //diganti manual
            fwrite($token_file, 'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJBUWNPM0V3MVBmQV9MQ0FtY2J6YnRLUEhtcWhLS1dRbnZ1VDl0RUs3akc4In0..GU1-5qeN14BmoLDmUq7b69L94qFkNppHYn1NgqNe-wkJunK2dlvZNMxSGbwnV7mNxX4wYBFVRYoX-y5S6N0nTqb_TogjVLyCbcpx2bZoDZK-ArQ7vGPi4O0nDl05vO8hljqkts6E9TGTXOjnW-x4LZ-TGt8m9Sw-id7CFOLjxWg765tsCPdGdKnH0UZ8Q08QjwLhCrqYyK0hIdYkxv6ra0fcQRxojyBTmMkulWvO9UPcua50dKPB59qaikE0mWpvP3ssz9zhLH6GNwdt3S0ECYj6ENFgtcJnQcfO_9ZEDPztzPCwxH9tbo1raExGGB6KoBvqk-ocLFkNE7G8O4lLBw');
            //fwrite($token_file, $obj['access_token']);
            fclose($token_file);
        }
    }
...
/*function untuk delete data riwayat siasn*/
function accessDatadeletesiasn($link = '', $param = '', $mode = 'prod')
{
    $lines = file('packages/tugumuda/token/token-key.txt');
    $kadaluarsa = strtotime(date($lines[0]));
    $skrg = strtotime(date('Y-m-d H:i:s'));
    ...
    $resultApi = apiResultDelete($base_url . $link . '/' . $param);
    $obj = json_decode($resultApi);
