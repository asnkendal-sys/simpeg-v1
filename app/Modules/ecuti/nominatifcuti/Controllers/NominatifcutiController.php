...
class NominatifcutiController extends Controller {
	...
	$input['tgl_usul'] = date("Y-m-d", strtotime(Input::get('tgl_usul')));
        ...
        $date2  =date_create($input['tgl_mulai']);
		...
		$rssimpan = 0;
                // exit();
                echo ($rssimpan)?1:"Maksimal Tanggal Untuk Pengusulan Cuti Adalah berjarak 10 Hari Kalenders";
        } elseif ($jarak >= -10) {
    ...
    public function postSimpandaripegawai(){
        ...
        if ($validation->passes()){
            ...
            // echo "<br> ".$jarak."<br> ".abs($jarak);
             if ($jarak < -10) {
                 // $ret['notif'] = 1;
                 $feedback = "<b>Perhatian!</b> Maksimal Tanggal Untuk Pengusulan Cuti Adalah berjarak 10 Hari Kalender";
                 // exit();
                 echo $feedback;
             } else {
                // $input['user_id'] = \Session::get('user_id');
...
public function postPenyesuaiankuota(){
    ...
    $nip = Input::get('nip');
    $input = Input::all();
    $cekkuota = \PenyesuaiankuotaModel::where('nip', $nip)->first();
    if(!empty($cekkuota)) {
        $penyu = \PenyesuaiankuotaModel::where('nip', $nip)
...
public function postCekkuotacuti(){
    ...
    if (count($rs) > 0) {
        ...
        if ($id_jenis_cuti == 1) {
            ...
            if ($mk_thn < 1 && \Session::get('role_id') > 3) {
...
//            }
            ...
            if (($mk_thn < 1 || ($pgw->idjenjab = 2 && substr($pgw->idjabfung, 0,3) == '300')) && \Session::get('role_id') > 3) {
                $feedback .="</tbody>
                </table>
                <b>Keterangan : </b> <br>
...
//            }
            ...
            $jum_anak = Hitanakkandung($nip,$pgw->tmtcpn,$pgw->tgskcalonawal_pppk,$pgw->idstspeg); //Fungsi untuk mendapatkan jumlah anak "KANDUNG"
...
public function postCekkuotabytanggalmulai(){
    ...
    if ($id_jenis_cuti == 1) {
        ...
        if ($rs->thn<$tahun_cuti) {
            $kuota_tahunan = (($rs->kuota_tahunan_n1<6)?0:6+(($rs->kuota_tahunan_n>6)?6:$rs->kuota_tahunan_n)+12);
        ...
        /*Kondisi jika YBS guru p3k tdk bisa ambil cuti tahunan*/
        if (($pgw->idstspeg == 3 && substr($pgw->idjabfung, 0,3) == '300') or ($pgw->idstspeg == 3 && substr($pgw->jabfung_id, 0,3) == '300')) {
            $ret['notif']       = 1;
            $ret['dis_mulai']   = 1;
            $ret['dis_selesai'] = 1;
            $ret['kuota_cuti']  = 0;
...
//        }
        ...
        if($tanggalmulai >= $tgl_bisa_besar){
            $ret['kuota_cuti']  = ((3*30.44) - $r_tahun->lama_cuti);
        ...
        if( ($mk_thn < 5) || ($r_tahun->lama_cuti != 0) || ($thn_bisa_besar <= $tahun_cuti) ){ //|| ($per_5thn!=5 && $tahun_cuti>$per_5thn)
            $feedback .="</tbody>
            </table>
            <b>Keterangan : </b> <br>
            <table class='table table-striped table-hover table-condensed table-bordered'>
        ...
        $jum_anak = Hitanakkandung($nip,$pgw->tmtcpn,$pgw->tgskcalonawal_pppk,$pgw->idstspeg); 
        $ret['kuota_cuti'] = $rs->kuota_melahirkan;
        if($jum_anak>=3){
            $ret['notif']       = 1;
            $ret['kuota_cuti']  = 0;
   ...
   //  }
    ...
    if ($id_jenis_cuti == 1) {
        ...
        if ($mk_thn < 1 && \Session::get('role_id') > 3) {
            ...
            <td>Masa Kerja</td>
            <td>1 Tahun</td>
            <td>".$mk_thn." Tahun ".$pgw->bln_cpns." Bulan ". $pgw->hari_cpns." Hari</td>
            <td class='text-center' style='vertical-align: middle;'>$x</td>
            </tr>";
        }else{
            $feedback .="<tr><td>1</td><td>Masa Kerja</td><td>1 Tahun</td><td>".getJarakDuaTanggal3((($pgw->idstspeg!=3)?$pgw->tmtcpn:$pgw->tgskcalonawal_pppk),date('Y/m/d'))."</td><td class='text-center' style='vertical-align: middle;'>$v</td></tr>";
        }
        /*Kondisi jika YBS sudah pernah menjalankan cuti Besar pada tahun berjalan*/
        /*HARUSNYA Letak Kondisi saat memilih tanggal mulai*/
        if ($tahun_cuti != 0) {   
...
//        }
        ...
        /*Kondisi jika YBS guru p3k tdk bisa ambil cuti tahunan*/
    // edit pppk guru boleh cuti candra
        // if (($pgw->idstspeg == 3 && substr($pgw->idjabfung, 0,3) == '300') or ($pgw->idstspeg == 3 && substr($pgw->jabfung_id, 0,3) == '300')) {
        //     $ret['notif']       = 1;
        //     $ret['dis_mulai']   = 1;
        //     $ret['dis_selesai'] = 1;
        ...
        if ($tahun_cuti == (($tahun_cuti!=0)?$r_besar->thn:"1") || $mk_thn < 1 || ($pgw->idjenjab = 2 && substr($pgw->idjabfung, 0,3) == '300')) {
            ...
            <td width='3%'></td>
            <td>
            <li>ASN yang menduduki jabatan guru pada sekolah dan jabatan dosen pada perguruan tinggi yang mendapat liburan menurut peraturan perundang-undangan, disamakan dengan ASN yang telah menggunakan hak cuti tahunan.</li>
            <li>*Jika terdapat kesalahan hubungi admin atau update biodata melalui <b>E-Personal</b>.</li>
            </td>
            </tr>
            </table>";
...
//        }
        ...
        if( ($mk_thn < 5) || ( (($tahun_cuti!=0)?$r_tahun->lama_cuti:"0") != 0) || ($thn_bisa_besar <= $tahun_cuti) ){ //|| ( (($tahun_cuti!=0)?$per_5thn:"0") !=5 && $tahun_cuti>(($tahun_cuti!=0)?$per_5thn:"0"))
            $feedback .="</tbody>
            </table>
            <b>Keterangan : </b> <br>
            <table class='table table-striped table-hover table-condensed table-bordered'>
     ...
     if(Input::get('tgl_mulai') != 0) {
         ...
         // echo "<br> ".$jarak."<br> ".abs($jarak);
          if ($jarak < -10) {
              ...
              $ret['dis_mulai']  = 0; //untuk menandai return untuk disabled tanggal mulai
              $ret['dis_selesai']  = 0;
             $feedback = "<b>Perhatian!</b> : Maksimal Tanggal Untuk Pengusulan Cuti Adalah berjarak 10 Hari Kalenderr";
             // exit();
         }
     }
