...
<head>
    ...
    <script type="text/javascript" src="{!!url()!!}/packages/tugumuda/plugins/jsbarcode/jquery.qrcode-0.11.0.js">
    </script>
</head>
...
<body>
    ...
    $template = TemplateskpengantarModel::getTemplate(session('idskpd') != '' ? session('idskpd') : 'all', '1.4', 'template');
    $where = "(tr_mutasi_dalam_skpd.statususul = 1 or tr_mutasi_dalam_skpd.statussk = 1)";
    $where .= "and tr_mutasi_dalam_skpd.nip =\"" . \Request::segment(6) . "\" and tr_mutasi_dalam_skpd.nousul = \"" . \Request::segment(5) . "\"";
    $count = \DB::table("tr_mutasi_dalam_skpd")->whereRaw($where)->count();


    $item = \DB::table('tr_mutasi_dalam_skpd')
        ->select(
            'tr_mutasi_dalam_skpd.*',
            'a_golruang.golru',
            'a_golruang.pangkat',
            'a_tkpendid.tkpendid',
        ...
        \DB::raw('IF(LEFT(skpdbaru.idskpd,2)=05 OR LEFT(skpdlama.idskpd,2)=04 OR LEFT(skpdlama.idskpd,2)=26,skpdbaru.path,CONCAT(skpdbaru.skpd," ",skpdbaru.path_short)) as skpdbaru'),*/
            \DB::raw('skpdlama.skpd as skpdlama'),
            \DB::raw('skpdbaru.skpd as skpdbaru'),
    ...
    $arrsearch = array(
        "search", "[no_suratrek]", "[surat_dari]", "[tgl_suratrek]", "[perihalrek]", "[nosk]", "[namalengkap]", "[nip]", "[tmlhr]", "[tglhr]", "[pangkat]", "[gol]", "[jablama]", "[skpdlama]", "[jabbaru]",
        "[skpdbaru]", "[tgl_sp]", "[jurusan]", "[tmt]", "[tglsurat]", "[namasekda]", "[pangkatsekda]", "[nipsekda]", "[kepalabkd]", "[pangkatbkd]", "[nipkepalabkd]", "[jabkepalabkd]", "[bupati]", "[tembusan_skpd_baru]", "[tembusan]", "[copyright]", "[qrcode]", "[tgl_berlaku]",
