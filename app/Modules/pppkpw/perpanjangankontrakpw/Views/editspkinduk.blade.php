...
if(Input::get('tipe') == 'all'){
    $rpppk = PerpanjangankontrakpwModel::whereRaw("sts_kontrak = 2 and MONTH(tr_pppkpw.tmtawal) = \"".Input::get('bulan')."\" and YEAR(tr_pppkpw.tmtawal) = \"".Input::get('tahun')."\"")->first();
}else if(Input::get('tipe') == 'kolektif'){
    $rpppk = PerpanjangankontrakpwModel::where('id', Input::get('id'))->where('sts_kontrak', 2)->first();
}
 ?>
<div>
...
<div>
    {!! Form::open(array('url' => url().'/pppkpw/perpanjangankontrakpw/simpanspkinduk', 'method' => 'POST', 'class'=>'form-horizontal form-'.\Config::get('claravel::ajax'),'id'=>'form-spk-induk')) !!}
...
<script type="text/javascript">
    ...
    $('#form-spk-induk').on('submit',function(e){
        e.preventDefault();
        $.ajax({
            url : '{!! url('') !!}/pppkpw/perpanjangankontrakpw/simpanspkinduk',
            type : 'post',
            data: $(this).serialize(),
            beforeSend: function(){
                preloader.on();
