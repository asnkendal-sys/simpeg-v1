<?php namespace App\Modules\webservices\skp\Controllers;

use App\Http\Controllers\Controller;
use App\Modules\webservices\apibkn\Models\ApibknModel;
use App\Modules\epersonal\biodata\Models\BiodataModel;
use App\Modules\webservices\skp\Models\SkpModel;
use App\Services\SkpService;
use Input,View, Request, Form, File;

class SkpController extends Controller {

	/**
	 * Skp Repository
	 *
	 * @var Skp
	 */
	protected $skp;

	public function __construct(BiodataModel $biodata)
	{
		$this->biodata = $biodata;
	}

	/**
	 * Display a listing of the resource.
	 *
	 * @return Response
	 */
	public function getIndex()
	{
		$where = ' tb_01.idjenkedudupeg not in (99,21)';
		if (session('role_id') > 3) {
				$where.= " and tb_01.idskpd like \"".session('idskpd')."%\" ";
		}
		if(Input::get('tahun')=='') {
				$where.=" and r_skp.tahun =".date('Y')."";
				//dd($where);
		}
		if ((strlen(Input::has('search')) > 0) or (Input::get('idskpd') != '') or (Input::get('idstspeg') != '') or (Input::get('tahun') != '')) {
				(Input::get('idstspeg')!='')?$where.=" and tb_01.idstspeg = '".Input::get('idstspeg')."'":"";
				(Input::get('idskpd')!='')?$where.=" and tb_01.idskpd LIKE '".Input::get('idskpd')."%'":"";
				(Input::get('search')!='')?$where.=" and (tb_01.nama LIKE '%".Input::get('search')."%' or tb_01.nip LIKE '%".Input::get('search')."%')":"";
				(Input::get('tahun')!='')? $where.=" and r_skp.tahun ='".Input::get('tahun')."'":"";

				if (session('role_id') <= 3) {
						$skps = $this->biodata
														->select(
																'tb_01.*',
																'a_golruang.golru',
																'a_skpd.path_short',
																'a_esl.esl',
																'a_tkpendid.tkpendid',
																'a_jenjurusan.jenjurusan',
																'r_skp.tahun',
																'r_skp.nilai',
																\DB::raw('CONCAT(tb_01.gdp,IF(LENGTH(tb_01.gdp)>0," ",""),tb_01.nama,IF(LENGTH(tb_01.gdb)>0,", "," "),tb_01.gdb) as namalengkap'),
																'a_jenkel.jenkel',
																'a_agama.agama',
																\DB::raw('IF(tb_01.idjenjab>4,a_skpd.jab,IF(tb_01.idjenjab=2,a_jabfung.jabfung,IF(tb_01.idjenjab=3,a_jabfungum.jabfungum,IF(tb_01.idjenjab=4,a_jabnonjob.jabnonjob,"-")))) as jabatan'),
																\DB::raw("
										CONCAT(
												IF((LEFT(tb_01.idgolrupkt,1) != LEFT(idgolrucpn,1)),
														(SUBSTR(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0,1,
																(LENGTH(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0)-2))
																-
																(IF((LEFT(tb_01.idgolrupkt,1) >= 3 AND LEFT(idgolrucpn,1) = 1), 11 - tb_01.mkthncpn,
																IF((LEFT(tb_01.idgolrupkt,1) >= 3 AND LEFT(idgolrucpn,1) = 2), 5 - tb_01.mkthncpn,
																		IF((LEFT(tb_01.idgolrupkt,1) = 2 AND LEFT(idgolrucpn,1) = 1), 6 - tb_01.mkthncpn, 0 ))))
														),
														(SUBSTR(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0,1,
																(LENGTH(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0)-2))
																+ tb_01.mkthncpn
														)
												),
												RIGHT(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0, 2)) AS mkskr
								"),
																\DB::raw("DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(tb_01.tglhr)), '%Y%m')+0 AS usia")
														)
														->leftjoin('a_skpd', 'tb_01.idskpd', '=', 'a_skpd.idskpd')
														->leftjoin('a_esl', 'tb_01.idesljbt', '=', 'a_esl.idesl')
														->leftjoin('a_tkpendid', 'tb_01.idtkpendid', '=', 'a_tkpendid.idtkpendid')
														->leftjoin('a_jenjurusan', 'tb_01.idjenjurusan', '=', 'a_jenjurusan.idjenjurusan')
														->leftjoin('a_golruang', 'tb_01.idgolrupkt', '=', 'a_golruang.idgolru')
														->leftjoin('a_agama', 'tb_01.idagama', '=', 'a_agama.idagama')
														->leftjoin('a_jenkel', 'tb_01.idjenkel', '=', 'a_jenkel.idjenkel')
														->leftjoin('a_jabfung', 'tb_01.idjabfung', '=', 'a_jabfung.idjabfung')
														->leftjoin('a_jabfungum', 'tb_01.idjabfungum', '=', 'a_jabfungum.idjabfungum')
														->leftjoin('a_jabnonjob', 'tb_01.idjabnonjob', '=', 'a_jabnonjob.idjabnonjob')
														->leftjoin('r_skp', 'tb_01.nip', '=', 'r_skp.nip')
														->whereRaw($where)
														->orderBy(\DB::raw('tb_01.idgolrupkt desc,tb_01.tmtpkt,tb_01.idjenjab,tb_01.idesljbt,tb_01.tmtesljbt,tb_01.tmtcpn,tb_01.idtkpendid desc,tb_01.thijaz,tb_01.tglhr'))
														->paginate($_ENV['configurations']['list-limit']);
				} else {

						$skps = $this->biodata
														->select(
																'tb_01.*',
																'a_golruang.golru',
																'a_skpd.path_short',
																'a_esl.esl',
																'a_tkpendid.tkpendid',
																'a_jenjurusan.jenjurusan',
																\DB::raw('CONCAT(tb_01.gdp,IF(LENGTH(tb_01.gdp)>0," ",""),tb_01.nama,IF(LENGTH(tb_01.gdb)>0,", "," "),tb_01.gdb) as namalengkap'),
																'a_jenkel.jenkel',
																'a_agama.agama',
																'r_skp.tahun',
																'r_skp.nilai',
																\DB::raw('IF(tb_01.idjenjab>4,a_skpd.jab,IF(tb_01.idjenjab=2,a_jabfung.jabfung,IF(tb_01.idjenjab=3,a_jabfungum.jabfungum,IF(tb_01.idjenjab=4,a_jabnonjob.jabnonjob,"-")))) as jabatan'),
																\DB::raw("
												CONCAT(
														IF((LEFT(tb_01.idgolrupkt,1) != LEFT(idgolrucpn,1)),
																(SUBSTR(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0,1,
																		(LENGTH(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0)-2))
																		-
																		(IF((LEFT(tb_01.idgolrupkt,1) >= 3 AND LEFT(idgolrucpn,1) = 1), 11 - tb_01.mkthncpn,
																		IF((LEFT(tb_01.idgolrupkt,1) >= 3 AND LEFT(idgolrucpn,1) = 2), 5 - tb_01.mkthncpn,
																				IF((LEFT(tb_01.idgolrupkt,1) = 2 AND LEFT(idgolrucpn,1) = 1), 6 - tb_01.mkthncpn, 0 ))))
																),
																(SUBSTR(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0,1,
																		(LENGTH(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0)-2))
																		+ tb_01.mkthncpn
																)
														),
														RIGHT(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0, 2)) AS mkskr
										"),
																\DB::raw("DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(tb_01.tglhr)), '%Y%m')+0 AS usia")
														)
														->join('a_skpd', 'tb_01.idskpd', '=', 'a_skpd.idskpd')
														->leftjoin('a_esl', 'tb_01.idesljbt', '=', 'a_esl.idesl')
														->leftjoin('a_tkpendid', 'tb_01.idtkpendid', '=', 'a_tkpendid.idtkpendid')
														->leftjoin('a_jenjurusan', 'tb_01.idjenjurusan', '=', 'a_jenjurusan.idjenjurusan')
														->leftjoin('a_golruang', 'tb_01.idgolrupkt', '=', 'a_golruang.idgolru')
														->leftjoin('a_agama', 'tb_01.idagama', '=', 'a_agama.idagama')
														->leftjoin('a_jenkel', 'tb_01.idjenkel', '=', 'a_jenkel.idjenkel')
														->leftjoin('a_jabfung', 'tb_01.idjabfung', '=', 'a_jabfung.idjabfung')
														->leftjoin('a_jabfungum', 'tb_01.idjabfungum', '=', 'a_jabfungum.idjabfungum')
														->leftjoin('a_jabnonjob', 'tb_01.idjabnonjob', '=', 'a_jabnonjob.idjabnonjob')
														->leftjoin('r_skp', 'tb_01.nip', '=', 'r_skp.nip')
														->whereRaw($where)
														->orderBy(\DB::raw('tb_01.idgolrupkt desc,tb_01.tmtpkt,tb_01.idjenjab,tb_01.idesljbt,tb_01.tmtesljbt,tb_01.tmtcpn,tb_01.idtkpendid desc,tb_01.thijaz,tb_01.tglhr'))
														->paginate($_ENV['configurations']['list-limit']);
				}
		} else {
				$skps = $this->biodata
												->select(
														'tb_01.*',
														'a_golruang.golru',
														'a_skpd.path_short',
														'a_esl.esl',
														'a_tkpendid.tkpendid',
														'a_jenjurusan.jenjurusan',
														\DB::raw('CONCAT(tb_01.gdp,IF(LENGTH(tb_01.gdp)>0," ",""),tb_01.nama,IF(LENGTH(tb_01.gdb)>0,", "," "),tb_01.gdb) as namalengkap'),
														'a_jenkel.jenkel',
														'a_agama.agama',
														'r_skp.tahun',
														'r_skp.nilai',
														\DB::raw('IF(tb_01.idjenjab>4,a_skpd.jab,IF(tb_01.idjenjab=2,a_jabfung.jabfung,IF(tb_01.idjenjab=3,a_jabfungum.jabfungum,IF(tb_01.idjenjab=4,a_jabnonjob.jabnonjob,"-")))) as jabatan'),
														\DB::raw("
										CONCAT(
												IF((LEFT(tb_01.idgolrupkt,1) != LEFT(idgolrucpn,1)),
														(SUBSTR(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0,1,
																(LENGTH(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0)-2))
																-
																(IF((LEFT(tb_01.idgolrupkt,1) >= 3 AND LEFT(idgolrucpn,1) = 1), 11 - tb_01.mkthncpn,
																IF((LEFT(tb_01.idgolrupkt,1) >= 3 AND LEFT(idgolrucpn,1) = 2), 5 - tb_01.mkthncpn,
																		IF((LEFT(tb_01.idgolrupkt,1) = 2 AND LEFT(idgolrucpn,1) = 1), 6 - tb_01.mkthncpn, 0 ))))
														),
														(SUBSTR(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0,1,
																(LENGTH(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0)-2))
																+ tb_01.mkthncpn
														)
												),
												RIGHT(DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(IF(tb_01.tmtcpn='0000-00-00',tb_01.tmtpns,tb_01.tmtcpn))), '%Y%m')+0, 2)) AS mkskr
								"),
														\DB::raw("DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(tb_01.tglhr)), '%Y%m')+0 AS usia")
												)
												->join('a_skpd', 'tb_01.idskpd', '=', 'a_skpd.idskpd')
												->leftjoin('a_esl', 'tb_01.idesljbt', '=', 'a_esl.idesl')
												->leftjoin('a_tkpendid', 'tb_01.idtkpendid', '=', 'a_tkpendid.idtkpendid')
												->leftjoin('a_jenjurusan', 'tb_01.idjenjurusan', '=', 'a_jenjurusan.idjenjurusan')
												->leftjoin('a_golruang', 'tb_01.idgolrupkt', '=', 'a_golruang.idgolru')
												->leftjoin('a_agama', 'tb_01.idagama', '=', 'a_agama.idagama')
												->leftjoin('a_jenkel', 'tb_01.idjenkel', '=', 'a_jenkel.idjenkel')
												->leftjoin('a_jabfung', 'tb_01.idjabfung', '=', 'a_jabfung.idjabfung')
												->leftjoin('a_jabfungum', 'tb_01.idjabfungum', '=', 'a_jabfungum.idjabfungum')
												->leftjoin('a_jabnonjob', 'tb_01.idjabnonjob', '=', 'a_jabnonjob.idjabnonjob')
												->leftjoin('r_skp', 'tb_01.nip', '=', 'r_skp.nip')
												->whereRaw($where)
												->orderBy(\DB::raw('tb_01.idgolrupkt desc,tb_01.tmtpkt,tb_01.idjenjab,tb_01.idesljbt,tb_01.tmtesljbt,tb_01.tmtcpn,tb_01.idtkpendid desc,tb_01.thijaz,tb_01.tglhr'))
												->paginate($_ENV['configurations']['list-limit']);
		}


		return View::make('skp::index', compact('skps'));

	}

		/**
	 * Show the form for creating a new resource.
	 *
	 * @return Response
	 */
	public function getCreate()
	{
		return View::make('skp::create');
	}

	public function getAmbildata()
	{
		return View::make('skp::ambil-data');
	}
	public function getPreview(SkpService $srv)
	{
		$input = Input::all();
		// dd($input);
		$skp= (array)\DB::table('r_skp')->where('nip', $input['nip'])->where('tahun', $input['tahun'])->first();

		return View::make('skp::preview', compact('skp'));
	}
	public function getListambilskp(SkpService $srv)
	{
		$input = Input::all();
		$skp = $srv->fetch($input['nip'])->filter('tahun',$input['tahun'])->first();

		return View::make('skp::listambilskp', compact('skp'));
	}

	public function postSave(SkpService $srv)
	{
		$input = Input::all();
		
		$data = [
			'nip' => $input['nip'],
			'tahun' => $input['tahun'],
			'idskpbkn' => $input['id_skp'],
			// 'idskpd' => $input['idskpd'],
			'nilai' => $input['nilaiSkp'],
			'orpel' => $input['orientasiPelayanan'],
			'integritas' => $input['integritass'],
			'komitmen' => $input['komitmens'],
			'disiplin' => $input['disiplins'],
			'kerjasama' => $input['kerjasamas'],
			'pim' => $input['kepemimpinans'],
			// 'nilaiprestasi' => $input['nilaiPrestasiKerja'],
			// 'nilaiperilaku' => $input['nilaiPerilakuKerja'],
			// 'nilairatarata' => $input['nilairatarata'],
			// 'pejpenilai' => $input['penilaiNama'],
			// 'nippenilai' => $input['penilaiNipNrp'],
			'statusPenilai' => 'pnss'
		];

		$skpexists = \DB::table('r_skp')->where('tahun', $input['tahun'])->where('idskpbkn',$input['id_skp'])->exists();
		if($skpexists) {
				$save = \DB::table('r_skp')->where('idskpbkn',$input['id_skp'])->update($data);
			
		}else{
		  	$save = \DB::table('r_skp')->insert($data);
				$save = ($save)?1:"Gagal Disimpan";
		}

	  echo $save;
	}

	public function postSend(SkpService $srv)
	{
		$input = Input::all();
		
		$data = array(
				'id' => $input['id_skp'],
				'tahun' => $input['tahun'],
				'nilaiSkp' => floatval($input['nilai']),
				'orientasiPelayanan' =>  floatval($input['orpel']),
				'integritas' =>  floatval($input['integritas']),
				'komitmen' =>  floatval($input['komitmen']),
				'disiplin' =>  floatval($input['disiplin']),
				'kerjasama' =>  floatval($input['kerjasama']),
				'nilaiPerilakuKerja' =>   80,
				'nilaiPrestasiKerja' =>  80,
				'kepemimpinan' =>  floatval($input['pim']),
				'jumlah' =>  80,
				'nilairatarata' => 80,
				'atasanPejabatPenilai' => '',
				'pejabatPenilai' => '',
				'pnsDinilaiOrang' => 'A8ACA7B755C43912E040640A040269BB',
				'penilaiNipNrp' => '',
				'atasanPenilaiNipNrp' => '',
				'penilaiNama' => '',
				'atasanPenilaiNama' => '',
				'penilaiUnorNama' => '',
				'atasanPenilaiUnorNama' => '',
				'penilaiJabatan' => '',
				'atasanPenilaiJabatan' => '',
				'penilaiGolongan' => '',
				'atasanPenilaiGolongan' => '',
				'penilaiTmtGolongan' => '',
				'atasanPenilaiTmtGolongan' => '',
				'statusPenilai' => 'PNS',
				'statusAtasanPenilai' => 'PNS',
				'jenisJabatan' => '2',
				 'pnsUserId' => 'A8ACA7B755C43912E040640A040269BB'
		);
		// dd($data);

			$save = $srv->store($data);
	
			$save = ($save['message'] ==true)?1:'terjadi kesalahan';


		echo $save;
	}
}
