...
if (session('role_id') <= 3) {
    ...
    $alert = "<strong>Perhatian!</strong><ul><li>Penambahan, Update atau Delete akan diverifikasi oleh Admin BKD terlebih dahulu.</li><li>Riwayat diklat Teknis yang memiliki tmt terbaru akan langsung terupdate ke biodata.</li><li>HARAP MENUNGGU VERIFIKASI DARI ADMIN BKPP DAN ADMIN SIASN BKN UNTUK UPDATE NILAI IP ASN</li><li>MOHON DICEK SECARA BERKALA</li></ul>";
}
?>
...
<div class="nav-tabs-custom" style="box-shadow:none;">
    ...
    <div class="tab-content">
        <div id="tab_1" class="tab-pane active">
            <div class="row">
                <div class="col-md-12">
                    {!! Form::open(array('url' => url()."/epersonal/biodata/".((session('role_id') <= 3)?'saverdiktek':'saverdiktektemp'), 'method'=> 'POST', 'files' => true, 'class'=>'form-horizontal form-'.\Config::get('claravel::ajax'),'id'=>'form-rdiktek')) !!}
                        @if(session('role_id') <= 3)
                            {!! Form::hidden('id', null, array('id'=> 'id')) !!}
                            @else
                            {!! Form::hidden('tb', Input::get('tb'), array('id'=> 'tb')) !!}
                            @if((Input::get('tb') == 'r_diktek') or (Input::get('tb') == ''))
                            {!! Form::hidden('id_rdiktek', null, array('id'=> 'id')) !!}
                            {!! Form::hidden('idjnsaksi', Input::get('flag'), array('id'=> 'idjnsaksi')) !!}
                            @else
                            {!! Form::hidden('id', null, array('id'=> 'id')) !!}
                            {!! Form::hidden('id_rdiktek', null, array('id'=> 'id_rdiktek')) !!}
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#form-rdiktek select').select2();
        $('#form-rdiktek .num').keyup(function() {
            if (this.value != this.value.replace(/[^0-9\.]/g, '')) {
                this.value = this.value.replace(/[^0-9\.]/g, '');
            }
        });
        $("#form-rdiktek .date").mask("99-99-9999");
        $("#form-rdiktek .datepicker").datetimepicker({
            format: 'DD-MM-YYYY'
        });
        $('#form-rdiktek').on('submit', function(e) {
            var $this = $(this);
            e.preventDefault();
        ...
        <?php if (Input::get("flag") > 1) { ?>
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (ret) {
                        for (attrname in ret) {
                            $('#form-rdiktek #' + attrname).val(ret[attrname]);
                            if ($.inArray(attrname, arrdate) != -1) {
                                var str = ret[attrname];
                                var res = str.split("-");
                                $('#form-rdiktek #' + attrname).val(res[2] + '-' + res[1] + '-' + res[0]);
                            }
                        }
                    }
                }
