<?php        
    ...
    $where = "status = 1 and sts_kontrak = 2 and  tmtawal = \"".$tmtawal."\" and idskpd like \"".$idskpd."%\"";

    $attr = \NominatifpensiunModel::attrPengantar($idskpd);
    $rs = \DB::table('tr_pppkpw')->whereRaw($where)->orderBy('status', 'desc');
    if($rs->count() > 0){
        $jml = $rs->count();
        $item = $rs->first();
    }else{
...
<form id="form-pengantar" class="form-horizontal" method="POST" action="{!!url()!!}/pppkpw/perpanjangankontrakpw/suratpengantar" target="_blank" accept-charset="UTF-8">
        ...
        <div class="box box-warning">
            ...
            <div class="box-body">
                <div class="col-md-12 data-biodata">
                    {!!csrf_field()!!}
                    <input type="hidden" name="tmtawal" id="tmtawal" value="{!!$tmtawal!!}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="controls col-sm-7">
                            <input type="text" name="berkas_pengantar" id="berkas_pengantar" class="form-control" placeholder="Jumlah berkas" readonly value="<?php echo $jml?>"> <?php //echo ($item->berkas_pengantar=='0')?$jml:$item->berkas_pengantar ?>
                        </div>
                    </div>
                    <div class="form-group">
