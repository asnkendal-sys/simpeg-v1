<?php
    ...
    /* Kondisi jangka ultah */
    if(Input::get('idwaktu') != ''){
        if(Input::get('idwaktu') < 0){
            $where .= " AND RIGHT(tb_01.tglhr, 5)=RIGHT(DATE_ADD(CURDATE(), INTERVAL ".Input::get('idwaktu')." DAY), 5)";
            $titlewk = substr(Input::get('idwaktu'), 1,1).' HARI LALU';
        }else if(Input::get('idwaktu') > 5){
            $where .= " AND MID(tb_01.tglhr, 6, 2)=MID(DATE_ADD(CURDATE(), INTERVAL +".substr(Input::get('idwaktu'), 1,1)." MONTH), 6, 2)";
            (Input::get('idwaktu')=='10')?$titlewk = 'BULAN INI':$titlewk = ' BULAN DEPAN';
        }else {
            $where .= " AND RIGHT(tb_01.tglhr, 5)=RIGHT(DATE_ADD(CURDATE(), INTERVAL +".Input::get('idwaktu')." DAY), 5)";
            (Input::get('idwaktu')=='0')?$titlewk = 'HARI INI':$titlewk = Input::get('idwaktu').' HARI LAGI';
        }
...
$rs = \DB::table('tb_01')
    ...
    $title = ((Input::get('idskpd') != '')?'PADA '.strtoupper(getSkpd(Input::get('idskpd')))." ".$titlewk:'');
     ...
     <Worksheet ss:Name=\"Sheet1\">
      <Table ss:ExpandedColumnCount=\"13000000\" ss:ExpandedRowCount=\"6000000\" x:FullColumns=\"1\"
       ...
       <Row ss:AutoFitHeight=\"0\">
        <Cell ss:MergeAcross=\"12\" ss:StyleID=\"s64\"><Data ss:Type=\"String\">DAFTAR NOMINATIF KENAIKAN PANGKAT REGULER</Data></Cell>
       </Row>
       <Row ss:AutoFitHeight=\"0\">
...
force_download("nominatif_penjagaan_ulangtahun_".date('Ymds').".xls", $w);
?>
