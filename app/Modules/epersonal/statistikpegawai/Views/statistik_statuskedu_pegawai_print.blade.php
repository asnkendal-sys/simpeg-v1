<html>
<head>
<title>{!!ucfirsts(getUtility('alias_aplikasi'))!!} Statistik Status Kedudukan Pegawai</title>
...
<?php
    $where = " tb_01.idjenkedudupeg='".Input::get('idjenkedudupeg')."' and tb_01.idstspeg in (".Input::get('idstspeg').")";
    $skpd = getSkpd(Input::get('idskpd'));
    $golru = getAttr('a_golruang', 'idgolru', Input::get('idgolru'), 'golru');
    $jenkedudupeg = getAttr('a_jenkedudupeg', 'idjenkedudupeg', Input::get('idjenkedudupeg'), 'jenkedudupeg');
...
<h3>BERDASARKAN STATUS KEDUDUKAN PEGAWAI {!!$jenkedudupeg!!}</h3>
</div><br>
    <table border="1">
