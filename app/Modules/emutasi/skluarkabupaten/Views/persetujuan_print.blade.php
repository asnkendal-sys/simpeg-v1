...
<head>
    ...
    <title>Surat Persetujuan Mutasi Luar</title>
...
<body>
    ...
    $template = \TemplateluarkabupatenModel::getTemplate('all','3.2','template');
    ...
    $arrsearch = array("search","[nosk]","[namalengkap]","[nip]","[tmlhr]","[tglhr]","[pangkat]","[gol]","[jablama]","[skpdlama]",
        "[pendidikan]","[jurusan]","[tmt]","[tglsurat]","[bupati]","[copyright]","[skpd]",

        "[kabinstansi]",
        "[tujuan_pem]",

        "[sekda]","[nipsekda]","[pangkatsekda]","[nopersetujuan]","[tglpersetujuan]","[nosp]","[tglsp]","[tembusan_kabinstansi]","[qrcode]","[nosk_persetujuan]","[tglsk_persetujuan]","[instansi_tujuan]","[kepala_instansi]","[nosurat]","[tgl_surat]","[jabpersetujuan]","[no_sp]","[tgl_sp]");
    $arrreplace = array("replace",$item->nosk,$item->namalengkap,fnip($item->nip),$item->tmlhr,formatTanggalPanjang($item->tglhr),ucword($item->pangkat),$item->golru,ucword($item->jabatan),ucword($item->path_short),
        $item->tkpendid,$item->jenjurusan,formatTanggalPanjang($item->tmt),formatTanggalPanjang($item->tglsurat),$item->bupati,$key_rand,$item->path_short,
        (($item->pemerintah!='')?$item->pemerintah.' ':'').$item->kabupaten,
        $tujuan_pem,
        ...
        (($item->nosk_persetujuan!='')?$item->nosk_persetujuan:'824.3/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/31'),(($item->tglsk_persetujuan!='0000-00-00')?formatTanggalPanjang($item->tglsk_persetujuan):''),$item->no_sp,formatTanggalPanjang($item->tgl_sp),
        $item->ditujukan_kepada,

        '<div id="qrcode"></div>',
        
        $item->nosk_persetujuan,formatTanggalPanjang($item->tglsk_persetujuan),
        
        $instansi_tujuan,

        $item->jab,$item->no_sp,formatTanggalPanjang($item->tgl_sp),$item->jabpengantar,$item->no_sp,formatTanggalPanjang($item->tgl_sp));
...
<script>
    $(document).ready(function(){
        ...
        $("#qrcode").qrcode({
            size    : 83.149606299,
            render  : "image",
            text	: "{!!url().'/digitalsignature/mutasi/luarkabupaten/persetujuan/'.$item->nousul.'/'.$item->nip!!}"
        });
