...
<head>
    ...
    <title>Surat Pengantar Persetujuan Mutasi Luar</title>
...
<body>
    ...
    <?php
        ...
        $template = \TemplateluarkabupatenModel::getTemplate('all','3.3','template');
        ...
        $arrsearch = array("search","[nosk]","[nourut]","[nip]","[namalengkap]","[tmlhr]","[tglhr]","[pangkat]","[gol]","[jablama]","[skpdlama]",
            "[pendidikan]","[jurusan]","[tmt]","[tglsurat]","[kepalabkd]","[pangkatbkd]","[nipbkd]","[bupati]","[copyright]","[nopersetujuan]","[tglpersetujuan]","[berkaspersetujuan]");
        $arrreplace = array("replace",(($item->nosk!='')?$item->nosk:'820/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/31'),$i,fnip($item->nip),$item->namalengkap,$item->tmlhr,formatTanggalPanjang($item->tglhr),$item->pangkat,$item->golru,$item->jabatan,$item->path,
            ...
            (($item->nosk_persetujuan!='')?$item->nosk_persetujuan:'822.3/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/'.date('Y')),(($item->tglsk_persetujuan!='0000-00-00')?formatTanggalPanjang($item->tglsk_persetujuan):formatTanggalPanjang(date('Y-m-d'))),$count);
