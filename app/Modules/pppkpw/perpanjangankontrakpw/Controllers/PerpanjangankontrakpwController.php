...
namespace App\Modules\pppkpw\perpanjangankontrakpw\Controllers;

use App\Http\Controllers\Controller;
use App\Modules\pppkpw\perpanjangankontrakpw\Models\PerpanjangankontrakpwModel;
use Input, View, Request, Form, File, Session;
use PDF;

use App\Models\PPPK\RPPPK;
use App\Services\PPPKPWService;
use App\Models\PPPKPW\UsulanPPPKPW;

class PerpanjangankontrakpwController extends Controller
{

    protected $perpanjangankontrakpw;
    ...
    public function getIndex()
    {
        ...
        return View::make('perpanjangankontrakpw::index', compact('rpppk'));
    }
    ...
    public function getIndexnominatif()
    {
        ...
        if ((strlen(Input::has('search')) > 0) or (Input::get('idskpd') != '') or (Input::has('tahun') != '') or (Input::has('bulan') != '') or (Input::has('status') != '') or (Input::has('statussk') != '') or (Input::get('status_tte') != '')) {
            (Input::get('bulan') != '') ? $where .= " and MONTH(tr_pppkpw.tmtawal) = \"" . Input::get('bulan') . "\"" : "";
            (Input::get('tahun') != '') ? $where .= " and YEAR(tr_pppkpw.tmtawal) = \"" . Input::get('tahun') . "\"" : "";
            (Input::get('statussk') != '') ? $where .= " and tr_pppkpw.statussk = \"" . Input::get('statussk') . "\"" : "";
            (Input::get('status') != '') ? $where .= " and tr_pppkpw.status = \"" . Input::get('status') . "\"" : "";
            (Input::get('idskpd') != '') ? $where .= " and tr_pppkpw.idskpd LIKE '" . Input::get('idskpd') . "%'" : "";
            (Input::get('search') != '') ? $where .= " and (tr_pppkpw.nama LIKE '%" . Input::get('search') . "%' or tr_pppkpw.nip LIKE '%" . Input::get('search') . "%')" : "";
            ...
            $rpppk = PerpanjangankontrakpwModel::select('tr_pppkpw.*', 'r_tte.proses', \DB::raw("DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW())-TO_DAYS(tr_pppkpw.tglhr)), '%Y%m')+0 AS usia"))
        ...
        return View::make('perpanjangankontrakpw::index_nominatif', compact('rpppk'));
    }

    public function getCreate()
    {
        return View::make('perpanjangankontrakpw::create');
    }

    public function getCreateaps()
    {
        return View::make('perpanjangankontrakpw::createaps');
    }
    ...
    public function postCreate()
    {
        ...
        cekAjax();
        $input = Input::all();
        $validation = \Validator::make($input, PerpanjangankontrakpwModel::$rules);
        ...
        //     $data = array();
            foreach($nip as $key => $item){
                $attr = PerpanjangankontrakpwModel::getattpppkpw($item);
                ...
                $selisih_masakerja = $attr->mkthnakhir_pppk + $attr->selisih_tahunkerja;
                ...
                $temp_arr1 = array(                                        
                    ...
                    'kepalabkd' => getKepskpd('56','nama'),
                    'jabkepalabkd' => getKepskpd('56','jab'),
                    'nipkepalabkd' => getKepskpd('56','nip'),
                    'pangkatbkd' => getKepskpd('56','pangkat'),
                    'golrubkd' => getKepskpd('56','golru'),
            ...
            // exit();
            if (! empty($nip)){
                if(!\DB::table('tr_pppkpw')->insert($data)){
                    echo "Perpanjangan Kontrak PPPK PW gagal disimpan.";
                }else{
                    echo 1;
                }
            }else{
    ...
    function postData()
    {
        ...
        return View::make('perpanjangankontrakpw::' . $view . '_data');
    }

    function postPrint()
    {
        $view = Request::segment(4);
        return View::make('perpanjangankontrakpw::' . $view . '_print');
    }
    ...
    function getCetak()
    {
        ...
        if ($id == "all") {
            $item = PerpanjangankontrakpwModel::whereRaw("MONTH(tr_pppkpw.tmtawal) = \"" . $kode . "\" and YEAR(tr_pppkpw.tmtawal) = \"" . $tmtawal . "\"")
                ->where('sts_kontrak', '=', 2)->where('status', '=', 1)
                ->orderBy('tglhr')->orderBy('idgolru', 'desc')->orderBy('nama')
                ->get();
        } else if ($id == "kolektif") {
            $item = PerpanjangankontrakpwModel::where('tmtawal', '=', $tmtawal)
                ->where('sts_kontrak', '=', 2)->where('status', '=', 1)->where('kdunit', '=', $kode)
                ->orderBy('tglhr')->orderBy('idgolru', 'desc')->orderBy('nama')
                ->get();
        } else {
            $item = PerpanjangankontrakpwModel::where('id', '=', $id)
                ->where('sts_kontrak', '=', 2)->where('status', '=', 1)->where('nip', '=', $kode)
                ->orderBy('tglhr')->orderBy('idgolru', 'desc')->orderBy('nama')
                ->first();
        }

        return View::make('perpanjangankontrakpw::' . $view . '_print', compact('item', 'id', 'kode', 'tmtawal'));
        // }else{
        //     abort(404);
        // }
    }
    ...
    {
        $view = Request::segment(4);
        return View::make('perpanjangankontrakpw::' . $view . '_excel');
    }
    ...
    public function postDelete()
    {
        ...
        echo (\DB::table('tr_pppkpw')->where($data)->delete()) ? 9 : 'Gagal Dihapus';
    }

    /*function preview edit kgb*/
    function postEditpppkpw()
    {
        $idpppk = Input::get('idpppk');
        $nip = Input::get('nip');

        $rs = \DB::table('tr_pppkpw as a')
            ->select(
                'a.*',
                \DB::raw("
    ...
    public function postUpdatepppkpw()
    {
        cekAjax();
        $input = Input::all();
        ...
        $validation = \Validator::make($input, PerpanjangankontrakpwModel::$rule_updates);
        if ($validation->passes()) {
            $arrnot = array('', '_token');
            ...
            if (\DB::table('tr_pppkpw')->where($dt)->update($data)) {
                echo "4";
            } else {
    ...
    function postVerifikasipppkpw()
    {
        ...
        $dt['nip'] = $input['nip'];
        $dt['sts_kontrak'] = 2;
        $validation = \Validator::make($input, PerpanjangankontrakpwModel::$rule_verifikasi);
    ...
    {
        cekAjax();
        return View::make('perpanjangankontrakpw::editskinduk');
    }

    public function postSimpanskinduk()
    {
        cekAjax();
        $rpppk = PerpanjangankontrakpwModel::where('id', Input::get('id'))->first();
        if (!empty($rpppk)) {
            // Ambil data yang memenuhi kriteria, diurutkan berdasarkan tanggal lahir (asc)
            $rpppks = PerpanjangankontrakpwModel::where('sts_kontrak', 2)
                ->where('status', 1)
            ...
            foreach ($rpppks as $item) {
                $rpppks = PerpanjangankontrakpwModel::where('id', '=', $item->id)
                    ->update([
                        'nosk' => Input::get('nosk'),
                        'tgsk' => Input::get('tgsk'),
                        'no_urut' => $noUrut
    ...
    {
        cekAjax();
        return View::make('perpanjangankontrakpw::editspinduk');
    }
    ...
    public function postSimpanspinduk()
    {
        ...
        $rpppk = PerpanjangankontrakpwModel::find(Input::get('id'));
    ...
    {
        cekAjax();
        return View::make('perpanjangankontrakpw::editspkinduk');
    }
    ...
    public function postSuratpengantar()
    {
        ...
        if (($tmtawal != '') and ($idskpd != '')) {
            ...
            \DB::table('tr_pppkpw')->where('tmtawal', $tmtawal)->where('status', '=', 1)->where('sts_kontrak', '=', 2)->where('idskpd', 'like', '' . $idskpd . '%')->update($data);
            ...
            $item2 = PerpanjangankontrakpwModel::where('tmtawal', '=', $tmtawal)
                ->where('status', '=', 2)
                ->where('kdunit', '=', $kode)
            ...
            if (Input::get('actpengantar') == 1) {
                return View::make('perpanjangankontrakpw::pengantarsk_print', compact('item', 'item2', 'id', 'kode', 'tmtawal'));
            }

            if (Input::get('actnominatif') == 1) {
                return View::make('perpanjangankontrakpw::pengantarnom_print', compact('item', 'item2', 'id', 'kode', 'tmtawal'));
            }
        } else {
    ...
    function postView()
    {
        cekAjax();
        $view = Request::segment(4);
        return View::make('perpanjangankontrakpw::' . $view . '_view');
    }
        ...
        foreach ($_POST as $key => $value) {
            ...
            if (array_search($key, $arrnot) == "") {
                ...
                if (is_array($value)) {
                    ...
                    $dt['perpanjangan'] = PerpanjangankontrakpwModel::getKontrak($dt['tmtawal'], $dt['tmtakhir'], 1);
                    ...
                    // echo "</pre>";
                    // exit();
                    $rssimpan = \DB::table('tr_pppkpw')->insert($dt);
                }
            }
        }
    ...
    public function postAjukantte()
    {
        ...
        if (Input::has('nip') && Input::has('idpppk')) {
            $usulanPPPK = UsulanPPPKPW::where('nip', '=', Input::get('nip'))
                ->where('idpppk', '=', Input::get('idpppk'))->first();
            if (!empty($usulanPPPK)) {
                $s_pppk = new PPPKPWService($usulanPPPK);
                if ($s_pppk->generateSK('PPPKPW')) {
                    return $s_pppk->ajukanTTE('PPPKPW') ? 1 : "Gagal Mengajukan TTE!";
                }
            }
        }
    ...
    public function postCekberkas()
    {
        ...
        $cek = PerpanjangankontrakpwModel::cekDokumen($nip, $sts_kontrak);
