...
<?php
    $where = " tb_01.idjenkedudupeg not in (99,21) and tb_01.idjenjab = 2 and LEFT(tb_01.idjabfung,3) = '300' and tb_01.idstspeg in (".Input::get('idstspeg').")";
    $skpd = getSkpd(Input::get('idskpd'));
    $issek = getAttr('a_skpd', 'issek', Input::get('idissek'), 'issek');

    switch(Input::get('idissek')){
        case "notnull":
            $where .= " ";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ";
            break;
        case "null":
            $where .= " and a_skpd.issek='' ";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU YANG KOSONG ";
            break;
        case "1":
            $jenjang = 'TK';
            $where .= " and a_skpd.issek='1'";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ".strtoupper($jenjang);
            break;
        case "2":
            $jenjang = 'SD';
            $where .= " and a_skpd.issek='2'";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ".strtoupper($jenjang);
            break;
        case "3":
            $jenjang = 'SMP';
            $where .= " and a_skpd.issek='3'";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ".strtoupper($jenjang);
            break;
        case "4":
            $jenjang = 'DPK';
            $where .= " and a_skpd.issek='4'";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ".strtoupper($jenjang);
            break;
        case "5":
            $jenjang = 'SMA/SMK';
            $where .= " and a_skpd.issek='5'";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ".strtoupper($jenjang);
            break;
        default:
            $jenjang = 'UMUM (NON JENJANG PENDIDIKAN)';
            $where .= " and a_skpd.issek=''";
            $titleadd = " BERDASARKAN JABATAN FUNGSIONAL GURU ".strtoupper($jenjang);
            break;
    }
    ...
    switch(Input::get('idskpd')){
        case "":
            ...
            break;
        default:
            $where .= " and tb_01.idskpd = '".Input::get('idskpd')."'";
            break;
    }
    ...
    ->whereRaw($where)
    ->orderBy('a_skpd.idskpd', 'asc')
    //->groupBy('a_skpd.idskpd')

?>

<div align="center">
<h3>DAFTAR PEGAWAI PADA  {!!strtoupper($skpd)!!}</h3>
<h3> {!!$titleadd!!}</h3>
</div><br>
    <table border="1">
