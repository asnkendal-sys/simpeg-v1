...
<script type="text/javascript">
    //
    function clickModal(jenis,subjenis,syarat,nip,nama){
        claravel_modal('Preview Berkas Layanan','Loading...','main_modal');
        $.ajax({
            type:'post',
            url : '{!!url()!!}/epensiun/nominatifpensiun/data/prefile',
            data: {'nip': nip,'nama': nama, 'jenis': jenis, 'subjenis': subjenis, 'syarat': syarat, '_token' : '{!!csrf_token()!!}'},
        ...
        if(response.length > 0){
            response.forEach(item => {
                ...
                <tr>
                    ...
                    <td width="10%" class="text-center"><a href="javascript:void(0)" class="prefile btn btn-success" onclick="clickModal('${item.jenis}','${item.subjenis}','${item.syarat}','${nip}','${nama}')"><i class="fa fa-search"></i> Preview</a></td>
                </tr>
                `;
                }else{
                    html += `
    ...
    $(document).ready(function(){
        ...
        $('a.prefile').on('click', function(e){
            ...
            $.ajax({
                type:'post',
                url : '{!!url()!!}/epensiun/nominatifpensiun/data/prefile',
                data: {'nip': $(this).attr('recnip'),'nama': $(this).attr('recnama'), 'jenis': $(this).attr('recjenis'), 'subjenis': $(this).attr('recsubjenis'), 'syarat': $(this).attr('recsyarat'), '_token' : '{!!csrf_token()!!}'},
                success:function(html){
                    $('#main_modal .modal-body').html(html);
                }
            });
