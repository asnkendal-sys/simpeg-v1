...
<body>
  ...
  <?php
    ...
    $rs =  \DB::table('tr_kenaikan_pangkat')
            ...
            \DB::raw('IF(tb_01.idjenjab>4,a_skpd.jab,IF(tb_01.idjenjab=2,a_jabfung_lama.jabfung,IF(tb_01.idjenjab=3,a_jabfungum_lama.jabfungum,"-"))) as jabatan_lama')
        )
        ...
        ->leftjoin('a_jabfung AS a_jabfung_lama', 'tr_kenaikan_pangkat.idjabfung_lama', '=', 'a_jabfung_lama.idjabfung') //
        ->leftjoin('a_jabfungum AS a_jabfungum_lama', 'tr_kenaikan_pangkat.idjabfungum_lama', '=', 'a_jabfungum_lama.idjabfungum') //
        ->whereRaw($where)
        ->get();
    // dd($rs); die();
  ?>
  ...
  <div id="F4-landscape">
    <table border="0" cellpadding="0" width="100%">
      <tr>
        <td><p><h3 align="center">DAFTAR NON NOMINATIF KENAIKAN PANGKAT</h3></p>
    ...
    <table class="table" border="1" cellpadding="0" width="100%" style="font-size:8pt">
        <thead>
            <tr>
                ...
                <th align="center" colspan="4" >Terakhir</th>
                <th align="center" colspan="4" >Diusulkan ke</th>
                <th align="center" rowspan="2" width="10%">Unit Kerja</th>
                <th align="center" colspan="3">Atasan Langsung</th>
            </tr>
            <tr>
                <th align="center" width="8%">Pangkat<br>Gol/Ruang<br>TMT</th>
                <th align="center" width="8%">Masa Kerja<br>Gaji Pokok</th>
                ...
                <th align="center" width="8%">Pangkat, Gol/Ruang</th>
                <th align="center" width="8%">Jabatan</th>
            </tr>
        </thead>
        <tbody>
            <?php $x=0; ?>
            ...
            <tr>
                ...
                <td>{!! $row->tkpendid."<br>".$row->jenjurusan !!}<br>{!!$row->thnlulus!!}</td>
                <td>{!! $row->jabatan_lama !!}<br>{!!$row->tmtjbt_lama!!}<br>{!! $row->nopaklama !!}</td> 
                <td>{!! $row->pangkatbaru !!}<br>{!! $row->golrubaru !!}<br>{!! tglina($row->tmt) !!}</td>
                <td>{!! $row->mktkpb." Thn ".$row->mkbkpb." Bln" !!} <br> {!!uang($row->gkpb)!!}</td>
                <td>{!! $row->tkpendid."<br>".$row->jenjurusan !!}<br>{!!$row->thnlulus!!}</td>
                <td>{!! $row->jabatan !!}<br>{!!$row->tmtjbt!!}<br>{!! $row->nopak !!}</td>
                <td>{!! $row->path_short !!}</td>
                <td>{!! $row->atasan_nama!!} <br> {!! $row->atasan_nip!!}</td>
                <td>{!! $row->atasan_pkt !!} <br> {!! $row->atasan_gol !!}</td>
