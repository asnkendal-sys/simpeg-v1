<?php namespace App\Modules\pppk\templateskpppk\Controllers;

use App\Http\Controllers\Controller;
use App\Modules\pppk\templateskpppk\Models\TemplateskpppkModel;
use Input,View, Request, Form, File;

class TemplateskpppkController extends Controller {

    /**
     * Templateskpppk Repository
     *
     * @var Templateskpppk
     */
    protected $templateskpppk;
    ...
    {
        cekAjax();
        return View::make('templateskpppk::index');
    }
    ...
    {
        return View::make('templateskpppk::create');
    }

    public function getTest()
    {
        echo TemplateskpppkModel::test();
    }
    ...
    function postData(){
        ...
        return View::make('templateskpppk::'.$view.'_data');
    }
    ...
    public function postSavetemplate(){
        ...
        $cek = \DB::table('tr_pppk_template')->where($dt)->count();
        if($cek === 0){
            ...
            $insert = \DB::table('tr_pppk_template')->insert($dt);
            ...
            $update = \DB::table('tr_pppk_template')->where($dt)->update($data);
    ...
    public function getDs($surat,$id)
    {
        $data = \App\Models\PPPK\RPPPK::findOrFail($id);
         return View::make('templateskpppk::digitalsign',compact('surat','data'));
    }
    ...
    function postSurat(){
        ...
        $rs = \DB::table('tr_pppk_template')->where(array('idskpd' => $idskpd, 'jnssurat' => $jnssurat))->get();
        if(count($rs) > 0){
            $rs['template'] = TemplateskpppkModel::getTemplate($idskpd, $jnssurat);
        }else{
            $rs['template'] = TemplateskpppkModel::getTemplate('all', $jnssurat);
        }
