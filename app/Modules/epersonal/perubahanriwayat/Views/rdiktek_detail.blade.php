<section class="content">
      ...
      {!! Form::open(array('url' => \Request::path().'/delete', 'method' => 'POST', 'class' => 'form-'.\Config::get('claravel::ajax'),'id'=>'data' )) !!}
                  ...
                  <p>
                     <div class="nav-tabs-custom" style="box-shadow:none;">
                        <ul class="nav nav-tabs tab2" id="myTabs">
                           ...
                           <li class="active"><a data-toggle="tab" href="#" id="rdiktek" recnip="{!! Input::get('nip')!!}"><i class="fa fa-fw fa-dot-circle-o"></i> DIKLAT TEKNIS</a></li>
                           ...
                           <div id="diktek" class="tab-pane active">
                              <p>
                                 <table class="table table-striped table-hover table-condensed table-bordered" role='grid' id="tb-rdiktek">
                                    ...
                                    @foreach ($rdikteks as $rdiktek)
                                    <?php $x++; ?>
                                    <tr>
                                        <td rowspan="2"align="center">{!!$x!!}</td>
                                        <td>{!!$rdiktek->nmdiktek!!}</td>
                                        <td>{!!$rdiktek->tmdiktek!!}</td>
                                        <td>{!!$rdiktek->penyelenggara!!}</td>
                                        <td>{!!$rdiktek->angkatan!!}</td>
                                        <td align="center">{!!date('d-m-Y',strtotime($rdiktek->tgmul))!!}</td>
                                        <td align="center">{!!date('d-m-Y',strtotime($rdiktek->tgsel))!!}</td>
                                        <td>{!!$rdiktek->jamhari!!} Jam</td>
                                        <td>{!!$rdiktek->nosttpdiktek!!}</td>
                                        <td align="center">{!!date('d-m-Y',strtotime($rdiktek->tgsttpdiktek))!!}</td>
                                        <td rowspan="2" align="right">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu pull-right">
                                                    <li><a class="text-info prevperubahan2" recid="{!!$rdiktek->id!!}" recnip="{!!$rdiktek->nip!!}" recflag="{!!$rdiktek->idjnsaksi!!}" href="javascript:void(0)" ><i class="{!!(session('role_id') <= 3)?'fa fa-check':'fa fa-search'!!}"></i> {!!(session('role_id') <= 3)?'Verifikasi':'Preview'!!}</a></li>
                                                    <li><a class="text-danger btlperubahan" recid="{!!$rdiktek->id!!}" recnip="{!!$rdiktek->nip!!}" recflag="{!!$rdiktek->idjnsaksi!!}" href="javascript:void(0)" ><i class="fa fa-times-circle"></i> Batalkan</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td><em><small>{!!$rdiktek->nip."<br>".$rdiktek->namalengkap!!}</small></em></td>
                                        <td colspan="3"><em><small>{!!$rdiktek->path!!}</small></em></td>
                                        <td colspan="5" class="{!!($rdiktek->status == 2)?'alert-danger':''!!}">
                                            <em><small>{!!getKetaksi($rdiktek->idjnsaksi)!!}</small></em><br>
                                            <em><small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Keterangan : {!!($rdiktek->ketditolak!='')?$rdiktek->ketditolak:'Belum ada tanggapan.'!!}</small></em>
                                        </td>
                                    </tr>
                                    @endforeach
                                    </tbody>
         ...
         <div class="box-footer clearfix">
            <div class="row">
               ...
               <div class="col-sm-6">
                   <?php echo $rdikteks->appends(array('idskpd'=>Input::get('idskpd'), 'search' => Input::get('search')))->render(); ?>
               </div>
            </div>
         </div>
      {!! Form::close() !!}
   ...
   <form id="prev-diktekakhir" class="form-horizontal">
      <div class="row ">
         <div class="col-md-12">
            <div class="box box-primary">
               <div class="box-body">
                  ...
                  <div class="nav-tabs-custom" style="box-shadow:none;">
                     <ul class="nav nav-tabs tab1" id="myTab2">
                        <li class="active"><a data-toggle="tab" href="#diktekakhir"> <i class="fa fa-fw fa-paper-plane"></i> DAFTAR DIKTEK</a></li>
                     </ul>
                     ...
                     <div class="tab-content data-awal">
                        <?php
                           ...
                           $rs2 = BiodataModel::getRdiktektemp($nip);
                        ?>
                        <div id="diktekakhir" class="tab-pane active">
                           <div class="row">
                              <table class="table table-striped table-hover table-condensed table-bordered" role='grid' id="tb-rdiktek">
                                  <thead class="bg-primary">
                                  <tr>
                                  ...
                                  <tbody id="result">
                                     ...
                                     @else
                                         <tr>
                                             <td colspan="11">Riwayat Diklat Teknis belum tersedia.</td>
                                         </tr>
                                     @endif
                                  </tbody>
                              </table>
...
<script>
   $(document).ready(function(){
      ...
      $('.prevperubahan2').on('click', function(e){
          e.preventDefault(e);
          claravel_modal('Perubahan Riwayat Diklat Teknis','Loading...','main_modal2');
          $.ajax({
             type:'post',
             url : '{!!url()!!}/epersonal/perubahanriwayat/data/rdiktek_perubahan',
          ...
          bootbox.confirm('Batalkan perubahan data ?',function(a){
             if(a == true){
                 $.ajax({
                      ...
                      type : 'post',
                      data: {'id' : $this.attr('recid'), 'tb':'r_diktek_temp', '_token' : '{!!csrf_token()!!}'},
                      beforeSend: function(){
                          preloader.on();
                      },
                      success:function(html){
