...
<body>
    ...
    $template = \DB::table('tr_mutasi_template_sk')->where('jnssurat','=','4.2')->where('idskpd','=','all')->first();
    ...
    $arrreplace = array("replace",formatTanggalPanjang($rs->tgl_sp),$rs->no_sp,$rs->berkas_sp,$rs->namalengkap,fnip($rs->nip),$sapaan_permohonan,$rs->namalengkap,formatTanggalPanjang($rs->tglsk_persetujuan),$rs->bupati,$rs->jabbupati,$rs->namalengkap,fnip($rs->nip),$rs->pangkat,$rs->golru,ucwords(strtolower($rs->jabatanlama)),
    	ucwords($rs->skpdlama),ucwords(strtolower($instansi)),ucwords(strtolower($rs->provinsi)),$rs->bupati,$rs->jabbupati,
    	ucwords(strtolower($rs->provinsi)),ucwords(strtolower($rs->kabupaten)),ucwords(strtolower($rs->provinsi)),ucwords(strtolower($rs->skpdlama)),ucwords(strtolower($rs->kabupaten)),$jab_kld,$kld,'<div id="qrcode"></div>','copyright',formatTanggalPanjang($rs->tgl_kajian),$rs->nosk,$rs->nosk2,$rs->nosk_persetujuan,$rs->nosk_persetujuan2,formatTanggalPanjang($rs->tglskpermintaan),$rs->tembusan);
