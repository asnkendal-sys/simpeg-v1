<html>
<head>
<title>{!!ucfirsts(getUtility('alias_aplikasi'))!!} Statistik Jenis Kelamin Eselon</title>
...
<?php
    $where = " tb_01.idjenkedudupeg not in (99,21) and tb_01.idjenkel='".Input::get('idjenkel')."' and tb_01.idjenjab >= 20 and tb_01.idstspeg in (".Input::get('idstspeg').")";
    $skpd = getSkpd(Input::get('idskpd'));
    $esl = getAttr('a_esl', 'idesl', Input::get('idesl'), 'esl');
    $jenkel = getAttr('a_jenkel', 'idjenkel', Input::get('idjenkel'), 'jenkel');
	...
	switch(Input::get('idesl')){
		case "notnull":
            $where .= " and tb_01.idesljbt != '' ";
			$titleadd = strtoupper($skpd)." SEMUA ESELON ";
		break;
		case "null":
            $where .= " and tb_01.idesljbt='' ";
			$titleadd = strtoupper($skpd)." YANG ESELONNYA KOSONG ";
		break;
		default:
            $where .= " and tb_01.idesljbt='".Input::get('idesl')."'";
			$titleadd = strtoupper($skpd)." ESELON ".$esl;
		break;
...
<h3>BERDASARKAN BERDASARKAN JENIS KELAMIN {!!(Input::get('idjenkel')=='1')?'LAKI-LAKI':'PEREMPUAN'!!}</h3>
</div><br>
    <table border="1">
