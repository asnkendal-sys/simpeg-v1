<section class="content">
      ...
      {!! Form::open(array('url' => \Request::path().'/delete', 'method' => 'POST', 'class' => 'form-'.\Config::get('claravel::ajax'),'id'=>'data' )) !!}
                  ...
                  <p>
                     <div class="nav-tabs-custom" style="box-shadow:none;">
                        <ul class="nav nav-tabs tab2" id="myTabs">
                           ...
                           <li class="active"><a data-toggle="tab" href="#" id="rdikfung" recnip="{!! Input::get('nip')!!}"><i class="fa fa-fw fa-dot-circle-o"></i> DIKLAT FUNGSIONAL</a></li>
                        ...
                        <div class="tab-content">
                           <div id="dikfung" class="tab-pane active">
                              <p>
                                 <table class="table table-striped table-hover table-condensed table-bordered" role='grid' id="tb-rdikfung">
                                    ...
                                    @foreach ($rdikfungs as $rdikfung)
                                    <?php $x++; ?>
                                    <tr>
                                        <td rowspan="2"align="center">{!!$x!!}</td>
                                        <td>{!!$rdikfung->dikfung!!}</td>
                                        <td>{!!$rdikfung->tmdikfung!!}</td>
                                        <td>{!!$rdikfung->penyelenggara!!}</td>
                                        <td>{!!$rdikfung->angkatan!!}</td>
                                        <td align="center">{!!date('d-m-Y',strtotime($rdikfung->tgmul))!!}</td>
                                        <td align="center">{!!date('d-m-Y',strtotime($rdikfung->tgsel))!!}</td>
                                        <td>{!!$rdikfung->jamhari!!} Jam </td>
                                        <td>{!!$rdikfung->nosttpdikfung!!}</td>
                                        <td align="center">{!!date('d-m-Y',strtotime($rdikfung->tgsttpdikfung))!!}</td>
                                        <td rowspan="2" align="right">
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu pull-right">
                                                    <li><a class="text-info prevperubahan2" recid="{!!$rdikfung->id!!}" recnip="{!!$rdikfung->nip!!}" recflag="{!!$rdikfung->idjnsaksi!!}" href="javascript:void(0)" ><i class="{!!(session('role_id') <= 3)?'fa fa-check':'fa fa-search'!!}"></i> {!!(session('role_id') <= 3)?'Verifikasi':'Preview'!!}</a></li>
                                                    <li><a class="text-danger btlperubahan" recid="{!!$rdikfung->id!!}" recnip="{!!$rdikfung->nip!!}" recflag="{!!$rdikfung->idjnsaksi!!}" href="javascript:void(0)" ><i class="fa fa-times-circle"></i> Batalkan</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td><em><small>{!!$rdikfung->nip."<br>".$rdikfung->namalengkap!!}</small></em></td>
                                        <td colspan="3"><em><small>{!!$rdikfung->path!!}</small></em></td>
                                        <td colspan="5" class="{!!($rdikfung->status == 2)?'alert-danger':''!!}">
                                            <em><small>{!!getKetaksi($rdikfung->idjnsaksi)!!}</small></em><br>
                                            <em><small>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Keterangan : {!!($rdikfung->ketditolak!='')?$rdikfung->ketditolak:'Belum ada tanggapan.'!!}</small></em>
                                        </td>
                                    </tr>
                                    @endforeach
                                    </tbody>
         ...
         <div class="box-footer clearfix">
            <div class="row">
               ...
               <div class="col-sm-6">
                   <?php echo $rdikfungs->appends(array('idskpd'=>Input::get('idskpd'), 'search' => Input::get('search')))->render(); ?>
               </div>
            </div>
         </div>
      {!! Form::close() !!}
   ...
   <form id="prev-dikfungakhir" class="form-horizontal">
      <div class="row ">
         <div class="col-md-12">
            <div class="box box-primary">
               <div class="box-body">
                  ...
                  <div class="nav-tabs-custom" style="box-shadow:none;">
                     <ul class="nav nav-tabs tab1" id="myTab2">
                        <li class="active"><a data-toggle="tab" href="#dikfungakhir"> <i class="fa fa-fw fa-paper-plane"></i> DAFTAR DIKFUNG</a></li>
                     </ul>
                     ...
                     <div class="tab-content data-awal">
                        <?php
                           ...
                           $rs2 = BiodataModel::getRdikfungtemp($nip);
                        ?>
                        <div id="dikfungakhir" class="tab-pane active">
                           <div class="row">
                              <table class="table table-striped table-hover table-condensed table-bordered" role='grid' id="tb-rdikfung">
                                  <thead class="bg-primary">
                                  <tr>
                                  ...
                                  <tbody id="result">
                                     ...
                                     @else
                                         <tr>
                                             <td colspan="11">Riwayat Diklat Fungsional belum tersedia.</td>
                                         </tr>
                                     @endif
                                  </tbody>
                              </table>
...
<script>
   $(document).ready(function(){
      ...
      $('.prevperubahan2').on('click', function(e){
          e.preventDefault(e);
          claravel_modal('Perubahan Riwayat Diklat Fungsional','Loading...','main_modal2');
          $.ajax({
             type:'post',
             url : '{!!url()!!}/epersonal/perubahanriwayat/data/rdikfung_perubahan',
          ...
          bootbox.confirm('Batalkan perubahan data ?',function(a){
             if(a == true){
                 $.ajax({
                      ...
                      type : 'post',
                      data: {'id' : $this.attr('recid'), 'tb':'r_dikfung_temp', '_token' : '{!!csrf_token()!!}'},
                      beforeSend: function(){
                          preloader.on();
                      },
                      success:function(html){
