<?php namespace App\Modules\kenaikangajiberkala\penetapannominatif\Models;
...
//    }
    ...
    public static function getDatanominatif($idkgb='', $jnskgb='', $piluptd=''){
        if($jnskgb != '0'){
    ...
    public static function getDatanominatifver($idkgb, $jnskgb){
        if($jnskgb != '0'){
...
//            ->first();
    ...
    public static function getJeniskgb($idgol='',$idskpd=''){
        $idgol = substr($idgol, 0, 1);
        if(substr($idskpd, 0, 2) == '04'){
            /*dinas pendidikan*/
            if($idgol == 4){
    ...
    public static function attrPengantar($idskpd){
        $rs = \DB::table("tb_01 as a")
            ->select('a.nip', \DB::raw("CONCAT(a.gdp,IF(LENGTH(a.gdp)>0,' ',''),a.nama,IF(LENGTH(a.gdb)>0,', ',''),a.gdb) AS nama"),
                'a.idskpd', 'a.idjenjab', 'a.idjabjbt', 'b.skpd', 'b.jab_utuh as jab', 'c.golru', 'c.pangkat')
            ->join('a_skpd as b', 'a.idjabjbt', '=', 'b.idskpd')
            ->leftJoin('a_golruang as c', 'a.idgolrupkt', '=', 'c.idgolru')
