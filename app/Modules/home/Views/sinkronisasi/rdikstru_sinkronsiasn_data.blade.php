...
<tr>
    ...
    @if($siasn->fileSiasn($item->path) != '')
        ...
        ?>
        <a class="text-info btn btn-success" href="{!!url().'/syncprevfile/kursus/'.$nip.'/'.$item->id.'/'.$siasn->clean($item->latihanStrukturalNama)!!}" target="_blank"><i class="fa fa-arrow-circle-o-down"></i> Download File</a>
    @else
        Dokumen Tidak Tersedia
    @endif
    </td>
    <td class="text-center">
        @if(cekBkn($item->id, 'r_dikstru'))
...
@else
<tr>
    <td colspan="8">Riwayat Diklat Struktural SIASN belum tersedia.</td>
</tr>
@endif
