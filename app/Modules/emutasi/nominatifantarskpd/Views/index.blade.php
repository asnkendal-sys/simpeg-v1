<section class="content-header">
	<h1>
		Nominatif Usulan Mutasi Antar OPD<small></small>
	</h1>
	<ol class="breadcrumb">
		<li><a href="{!!url()!!}"> Dashboard</a></li>
		<li class="active">Nominatif Usulan Mutasi Antar OPD</li>
	</ol>
</section>
			...
			<div class="box-body no-padding">
				<table class="table table-striped table-hover table-condensed table-bordered" id='tabel'>
					...
					<tbody>
						...
						$n = 0;
						?>
						@foreach ($nominatifantarskpds as $no => $nominatifantarskpd)
						<?php
						$n++;
							...
							<tr>
								<th style="position:relative;" colspan="6">
									<div class="text-left">
										...
										<i class="fa fa-calendar"></i> <?php echo date("d-m-Y", strtotime($nominatifantarskpd->tglusul))?>
										&nbsp;
										<?php
										if ($nominatifantarskpd->idskpd!="") {
										 	$ket = "";
										 } else{
										 	$ket = "Penempatan CPNS";
										 }
										echo "||&nbsp;".getskpdgroup(substr($nominatifantarskpd->idskpd, 0,2))." ".$ket; 
										?>&nbsp;
										<br>
									</div>
								</th>
								<th style="position:relative;" colspan="6">
									<div class="text-right">
										...
										recnousul="{!!$nominatifantarskpd->nousul!!}" 
										rectglusul="{!!$nominatifantarskpd->tglusul!!}" 
										recnosuratrek="{!!$nominatifantarskpd->no_suratrek!!}" 
										recsuratdari="{!!$nominatifantarskpd->surat_dari!!}" 
										rectglsurat="{!!$nominatifantarskpd->tgl_suratrek!!}"
										recperihalrek="{!!$nominatifantarskpd->perihalrek!!}" 
										...
										&nbsp;<a href="{!!url()!!}/emutasi/nominatifantarskpd/cetak/nominatif/{!!$nominatifantarskpd->nousul!!}/{!!$nominatifantarskpd->tglusul!!}" target="_blank" recnousul="{!!$nominatifantarskpd->nousul!!}" title="Cetak Daftar Nominatif"><i class="fa fa-list"> Nominatif</i></a> |
						...
						<tr>
							...
							<td>{!!$nominatifantarskpd->nip!!}<br>{!!$nominatifantarskpd->namalengkap!!}
								...
								<td>{!!$nominatifantarskpd->golru!!}<br>{!!($nominatifantarskpd->tmtpkt!='0000-00-00')?date('d-m-Y', strtotime($nominatifantarskpd->tmtpkt)):''!!}</td>
								<td>{!!$nominatifantarskpd->tkpendid!!}<br>{!!$nominatifantarskpd->jenjurusan!!}</td>
								<td>{!!$nominatifantarskpd->skpdlama!!}</td>

								<td>{!!$nominatifantarskpd->jabatan!!}<br>{!!($nominatifantarskpd->tmtjbt!='0000-00-00')?date('d-m-Y', strtotime($nominatifantarskpd->tmtjbt)):''!!}</td>
											...
											<td>
												<div class="btn-group">
													...
													<ul class="dropdown-menu pull-right">
														<?php if($nominatifantarskpd->statususul!=1) {?>
														...
														<?php }else{ ?>
															<li><a href="javascript:void(0)" class="text-info" style="color: red" recidusul="{!!$nominatifantarskpd->idusul!!}" recnip="{!!$nominatifantarskpd->nip!!}" recnousul="{!!$nominatifantarskpd->nousul!!}"><i class="fa fa-pencil-square-o"></i>Edit Personal</a></li>
														<?php } ?>
														...
														<?php }else{?>
															<li><a href="javascript:void(0)" class="text-info verifikasimutasi" recidusul="{!!$nominatifantarskpd->idusul!!}" recnip="{!!$nominatifantarskpd->nip!!}" recnousul="{!!$nominatifantarskpd->nousul!!}"><i class="fa fa-search"></i> Preview Usulan</a></li>
														<?php }?>
						...
						<div class="box-footer clearfix">
							...
							<div class="col-sm-6">
								<?php echo $nominatifantarskpds->appends(array('idskpd' => Input::get('idskpd'), 'search' => Input::get('search')))->render(); ?>
							</div>
						</div>
						{!! Form::close() !!}
					</p>
...
<script>
	...
	$(document).ready(function(){
		...
		$('a.editmutasi').on('click', function(e){
			e.preventDefault();
			claravel_modal('Edit Nominatif Mutasi Antar SKPD','Loading...','main_modal2');
			$.ajax({
				type:'post',
		...
		$('a.verifikasimutasi').on('click', function(e){
			e.preventDefault();
			claravel_modal('Verifikasi Nominatif Mutasi Antar SKPD','Loading...','main_modal2');
			$.ajax({
				type:'post',
