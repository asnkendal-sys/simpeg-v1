...
<body>
    ...
    $arrreplace = array("replace",formatTanggalPanjang($rs->tgl_sp),$rs->no_sp,$sapaan_permohonan,$rs->namalengkap,$rs->skpdlama,$rs->berkas_sp,ucwords(strtolower($rs->kabupaten)),$sapaan_permohonan,$rs->namalengkap,formatTanggalPanjang($rs->tglsk_persetujuan),$rs->bupati,ucwords(strtolower($rs->provinsi)),ucwords(strtolower($rs->kabupaten)),ucwords(strtolower($rs->provinsi)),$rs->skpdlama,ucwords(strtolower($rs->kabupaten)),$jab_kld,$kld,'<div id="qrcode1"></div>','copyright',formatTanggalPanjang($rs->tgl_kajian),$rs->nosk_persetujuan,$rs->nosk_persetujuan2,$rs->tembusan);
...
<script>
    $(document).ready(function(){
        ...
        $("#qrcode").qrcode({
            size    : 90,
            render  : "image",
            text	: "{!!url().'/digitalsignature/mutasi/masukkabupaten/permohonan/'.$rs->nousul.'/'.$rs->nip!!}"
        });
