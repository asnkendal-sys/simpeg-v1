<?php        
    ...
    $rs = \DB::table('tr_pppk')->whereRaw($where)->orderBy('status', 'desc');
    if($rs->count() > 0){
        $jml = $rs->count();
        $item = $rs->first();
    }else{
...
<form id="form-pengantar" class="form-horizontal" method="POST" action="{!!url()!!}/pppk/perpanjangankontrak/suratpengantar" target="_blank" accept-charset="UTF-8">
