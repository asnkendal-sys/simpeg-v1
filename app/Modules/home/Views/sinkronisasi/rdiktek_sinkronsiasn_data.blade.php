...
@if(count($bkn) > 0)
    @foreach($bkn as $item)
        @if($item->jenisKursusSertifikat == 'DIKLAT TEKNIS')
        <?php $x++;?>
        <tr>
            ...
            <td class="text-center">
                @if($siasn->fileSiasn($item->path) != '')
                    ...
                    ?>
                    <a class="text-info btn btn-success" href="{!!url().'/syncprevfile/kursus/'.$nip.'/'.$item->id.'/'.$siasn->clean($item->namaKursus)!!}" target="_blank"><i class="fa fa-download"></i> Download File</a>
                @else
                    Dokumen Tidak Tersedia
                @endif
            </td>
            <td class="text-center">
                @if(cekBkn($item->id, 'r_diktek'))
    ...
    @if($x == 0)
    <tr>
        <td colspan="9">Riwayat Diktat Teknis SIASN belum tersedia.</td>
    </tr>
    @endif
@else
    <tr>
