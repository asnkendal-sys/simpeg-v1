...
<head>
    ...
    <title>Surat Pengantar Persetujuan Mutasi Masuk</title>
...
<body>
    ...
    <?php
        ...
        $template = \DB::table('tr_mutasi_template_sk')->where('jnssurat','=','4.3')->where('idskpd','=','all')->first();
        ...
        $arrsearch =  array("search","[tglpersetujuan]","[nopersetujuan]","[namalengkap]","[nip]","[berkaspersetujuan]","[kepalabkd]","[pangkatbkd]","[nipbkd]");

        $arrreplace = array("replace",(($rs->tglsk_persetujuan!='0000-00-00')?formatTanggalPanjang($rs->tglsk_persetujuan):formatTanggalPanjang(date('Y-m-d'))),(($rs->nosk_persetujuan!='')?$rs->nosk_persetujuan:'822.3/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/'.date('Y')),ucwords(strtolower($rs->namalengkap)),fnip($rs->nip),$jumlahberkas,$rs->kepalabkd,$rs->pangkatbkd,fnip($rs->nipkepalabkd));
