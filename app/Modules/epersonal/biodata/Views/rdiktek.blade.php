            ...
            <td align="right">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-right">
                        ...
                        <li><a class="text-info kelolafile" recid="{!!$item->id!!}" jns="Diklat Teknis {!!$item->nmdiktek!!}" href="javascript:void(0)" ><i class="fa fa-file-o"></i> Kelola File</a></li>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#rdiktek a.kelolafile').on('click',function(e){
            e.preventDefault();
            claravel_modal('Kelola File Riwayat Diklat Teknis','Loading...','kelola_file');
            var id = $(this).attr('recid');
            var nama_jenis = $(this).attr('jns');
            $.ajax({
                type:'post',
                url : '{!!url()!!}/epersonal/biodata/kelolafile',
                data: {'nip': '{!!$nip!!}', 'jenis': 11, 'subjenis': id, 'nama_jenis':nama_jenis, 'tb': 'r_gol', '_token' : '{!!csrf_token()!!}'},
        ...
        $('#rdiktek a.edit').on('click',function(e){
            e.preventDefault();
            ...
            $.ajax({
                ...
                data: {'nip': $('#nip').val(), 'id': id, 'flag':2, 'tb': 'r_diktek', '_token' : '{!!csrf_token()!!}'},
                success:function(html){
        ...
        $('#rdiktek a.editpermohonan').on('click',function(e){
            e.preventDefault();
            claravel_modal('Permohonan Riwayat Diklat Teknis','Loading...','main_modal');
            var id = $(this).attr('recid');
            $.ajax({
                type:'post',
                url : '{!!url()!!}/epersonal/biodata/data/rdiktek_form',
                data: {'nip': $('#nip').val(), 'id': id, 'flag':2, 'tb': 'r_diktek_temp', '_token' : '{!!csrf_token()!!}'},
                success:function(html){
                    $('#main_modal .modal-body').html(html);
                }
            });
        ...
        $('#rdiktek a.hapusmin').on('click',function(e){
            e.preventDefault();
            claravel_modal('Riwayat Diklat Teknis','Loading...','main_modal');
            var id = $(this).attr('recid');
            $.ajax({
                type:'post',
                url : '{!!url()!!}/epersonal/biodata/data/rdiktek_form',
                data: {'nip': $('#nip').val(), 'id': id, 'flag':3, 'tb': 'r_diktek', '_token' : '{!!csrf_token()!!}'},
                success:function(html){
                    $('#main_modal .modal-body').html(html);
                }
            });
        ...
        $('#rdiktek a.hapus').on('click',function(e){
            e.preventDefault();
            var id = $(this).attr('recid');
        ...
        $('#rdiktek a.hapuspermohonan').on('click',function(e){
            e.preventDefault();
            var id = $(this).attr('recid');
            bootbox.confirm('Hapus?',function(a){
                if(a == true){
                    $.ajax({
                        type:'post',
                        url : '{!!url()!!}/epersonal/biodata/delriwayat',
                        data: {'nip': $('#nip').val(), 'id': id, 'flag':3, 'tb':'r_diktek_temp', '_token' : '{!!csrf_token()!!}'},
                        beforeSend: function(){
                            preloader.on();
                        },
                        success:function(html){
        ...
        $('#rdiktek a.sinkronsiasn').on('click',function(e){
            e.preventDefault();
            claravel_modal('Sinkronisai SIASN','Loading...','main_modal2');
            var id = $(this).attr('recid');
            var iddiktekbkn = $(this).attr('idbkn');
            $.ajax({
                type:'post',
                url : '{!!url()!!}/sinkronsiasn/rdiktek_sinkronsiasn_form',
                data: {'nip': $('#nip').val(), 'id': id, 'iddiktekbkn': iddiktekbkn, 'flag':2, '_token' : '{!!csrf_token()!!}'},
                success:function(html){
        ...
        $('#rdiktek a.prevsinkronsiasn').on('click',function(e){
            e.preventDefault();
            claravel_modal('Preview Diklat Teknis SIASN','Loading...','main_modal2');
            var id = '';
            var iddiktekbkn = '';
            $.ajax({
                type:'post',
