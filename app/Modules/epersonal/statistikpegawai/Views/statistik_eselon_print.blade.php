...
<?php
    $where = " tb_01.idjenkedudupeg not in (99,21) and tb_01.idesljbt='".Input::get('idesl')."' and tb_01.idstspeg in (".Input::get('idstspeg').")";
    $skpd = getSkpd(Input::get('idskpd'));
    $golru = getAttr('a_golruang', 'idgolru', Input::get('idgolru'), 'golru');
    $eselon = getAttr('a_esl', 'idesl', Input::get('idesl'), 'esl');

    switch(Input::get('idskpd')){
        case "":
            $where .= "";
            $titleskpd = ' SEMUA SKPD';
            break;
        default:
            $where .= " and tb_01.idskpd like '".Input::get('idskpd')."%'";
            $titleskpd = strtoupper($skpd);
            break;
    }
    ...
    switch(Input::get('idgolru')){
        case "notnull":
            ...
            $titleadd = " SEMUA GOLONGAN ";
            break;
        case "null":
            $where .= " and tb_01.idgolrupkt='' ";
            $titleadd = " YANG GOLONGANNYA KOSONG ";
            break;
        default:
            $where .= " and tb_01.idgolrupkt='".Input::get('idgolru')."'";
            $titleadd = " GOLONGAN ".strtoupper($golru);
            break;
    }
...
?>
<div align="center">
<h3>DAFTAR PEGAWAI PADA  {!!$titleskpd!!}</h3>
<h3>BERDASARKAN BERDASARKAN ESELON {!!strtoupper($eselon)!!} DAN {!!$titleadd!!}</h3>
</div><br>
    <table border="1">
