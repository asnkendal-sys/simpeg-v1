...
class UsulanPPPK extends Model {
    ...
    {
        $pdf = PDF::loadHTML(View::make('perpanjangankontrak::skpetikan_tte',[
            'nip' => $nip,
            'idpppk' => $idpppk,
        ...
        $output = $pdf->stream();
        $file_path = 'pppk/draft/'.$nip.'_'.$idpppk.'_'.$sts_kontrak.'.pdf';
        ...
        if (Storage::disk('local')->put($file_path, $output)) {
            $tte = TTE::where('nip_pengusul',$nip)->where('jenis','PPPK')->where('id_sk',$idpppk)->first();
    ...
    {
        return $this->belongsTo('App\Models\Riwayat\TTE','idpppk','id_sk')->where('jenis','=','PPPK')->where('nip_pengusul','=',$this->nip);
    }
